...
Only `apps/property-data/**`, unless the user explicitly approves a broader scope.
...
- Canonical gate: `cd apps/property-data && ./gradlew backendQualityCheck`. It runs
  `test`, `persistenceTest`, and coverage checks, and it is the PR evidence
  command for any `apps/property-data/**` change
...
- During `docs/RESTRUCTURING_PLAN.md` execution the Gradle module layout
  changes (core/api/batch-app). Re-check task names and paths against the
