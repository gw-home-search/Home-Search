...
def required_skill_triggers_for_files(changed_files: Iterable[str]) -> set[str]:
    ...
    if any(path.startswith("apps/property-data/") and not is_removed_companion_doc(path) for path in changed):
    ...
    *,
    ...
    if backend_changed and require_pass:
        if not COVERAGE_LINE_RE.search(body):
            add(errors, "evidence", "apps/property-data 변경에는 `Coverage: >=90%` evidence가 필요합니다")
        if not DOCS_OPENAPI_LINE_RE.search(body):
            add(errors, "evidence", "apps/property-data 변경에는 `Docs/OpenAPI: generated + verified` evidence가 필요합니다")
...
def run_self_test() -> int:
    ...
    backend_missing_quality = valid_input(
        ...
        changed_files=("apps/property-data/core/src/main/java/com/home/App.java",),
    )
    backend_test_only = valid_input(
        body=valid_body().replace(
            f"- `{API_TEST}` = not run (api 변경 없음)",
            "- `cd apps/property-data && ./gradlew test` = pass (단위 테스트만 확인)",
        ),
        changed_files=("apps/property-data/core/src/main/java/com/home/App.java",),
    )
