...
def check_payload(
    ...
    *,
    ...
    if scope == "frontend" and any(path.startswith("apps/property-data/") for path in paths):
        deny("frontend worktree에서 apps/property-data/** 변경 차단.")
    ...
    tests = [
        ...
        (
            ...
            lambda: "rm -rf 차단" in denied_output(
                {"cwd": str(FALLBACK_REPO_ROOT), "tool_input": {"cmd": "rm -rf apps/property-data/build"}},
                repo_root=FALLBACK_REPO_ROOT,
                branch_name="feat/root",
            ),
        ),
        ...
        (
            "frontend worktree denies apps/property-data mutation",
            lambda: "apps/property-data/**" in denied_output(
                patch_payload(frontend_root, "apps/property-data/core/src/main/java/App.java"),
        ...
        (
            ...
            lambda: not denied_output(
                patch_payload(Path("/tmp/home-search-integration-work"), "apps/property-data/core/src/main/java/App.java"),
        ...
        (
            ...
            lambda: "build output 변경 차단" in denied_output(
                {
                    **patch_payload(FALLBACK_REPO_ROOT, "apps/property-data/build/generated.txt"),
                    "last_assistant_message": "\n".join(
                        [
                            "보호 경로 변경 요청:",
                            "보호 경로 대상: apps/property-data/build/generated.txt",
                            "보호 경로 변경 기준: current task approval only",
                            "사용자 승인: 확인",
                        ]
                    ),
