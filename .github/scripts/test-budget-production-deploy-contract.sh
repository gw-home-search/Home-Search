...
grep -Fq 'live_ml_desired_count="$(jq -er '\''.ml'\'' <<<"${live_application_desired_counts}")"' "${rollout_workflow}"
grep -Fq '[[ "${live_ml_desired_count}" =~ ^[0-9]+$ ]]' "${rollout_workflow}"
require_absent_literal '[[ "$(jq -r '\''.ml'\'' <<<"${live_application_desired_counts}")" == 0 ]]' "${rollout_workflow}"
