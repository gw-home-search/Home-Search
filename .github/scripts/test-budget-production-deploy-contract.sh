...
budget_observability_template="${root}/infra/terraform/budget-production/files/configure-observability.sh.tftpl"
...
grep -Fq -- '--query "reverse(sort_by(Contents[?ends_with(Key, '\''.manifest.tsv'\'')], &LastModified))[0].LastModified"' "${budget_observability_template}"
! grep -Fq 'ends_with(Key, `.manifest.tsv`)' "${budget_observability_template}"
