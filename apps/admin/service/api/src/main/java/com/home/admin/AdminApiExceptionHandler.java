...
public class AdminApiExceptionHandler {
    ...
    ProblemDetail notFound() { return ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, "관리자 계정을 찾을 수 없습니다."); }

    @ExceptionHandler({AdminAccountService.InvalidRoleException.class, IllegalArgumentException.class})
    ProblemDetail badRequest() { return ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, "요청 값을 확인하세요."); }

    @ExceptionHandler(DataIntegrityViolationException.class)
    ProblemDetail conflict() { return ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, "이미 존재하거나 현재 상태와 충돌합니다."); }

    @ExceptionHandler(AdminAccountService.CannotRemoveLastAdminException.class)
    ProblemDetail lastAdmin() { return ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, "마지막 active ADMIN은 변경할 수 없습니다."); }
