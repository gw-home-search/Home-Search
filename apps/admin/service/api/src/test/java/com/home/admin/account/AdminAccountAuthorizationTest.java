...
class AdminAccountAuthorizationTest {
    ...
    @MockitoBean AdminAccountService accountService;
    @MockitoBean AdminAuditService auditService;

    @Test
    void operatorCannotReadAccounts() throws Exception {
        mvc.perform(get("/api/v1/admin/accounts").with(authentication(authenticationFor("OPERATOR", "COORDINATE_READ"))))
    ...
    void accountManagerCanReadAccounts() throws Exception {
        ...
        mvc.perform(get("/api/v1/admin/accounts").with(authentication(authenticationFor("ADMIN", "ADMIN_ACCOUNT_MANAGE"))))
    ...
    void accountMutationRequiresCsrf() throws Exception {
        ...
        String body = "{\"loginId\":\"viewer\",\"displayName\":\"조회자\",\"password\":\"long-test-password\",\"roles\":[\"VIEWER\"]}";
        mvc.perform(post("/api/v1/admin/accounts").with(authentication(authentication))
    ...
    private UsernamePasswordAuthenticationToken authenticationFor(String role, String permission) {
        AdminPrincipal principal = new AdminPrincipal(UUID.randomUUID(), "actor", "관리자", Set.of(role), Set.of(permission));
