...
class RestClientPropertyAdminClientTest {
    ...
    void sendsSignedActorRequestAndPreservesDownstreamProblemResponse() throws Exception {
        ...
        var issuer = new InternalAdminTokenIssuer(
                codec, keys.getPrivate(), "active-key", "admin-service", "property-data-admin", Duration.ofSeconds(60));
                ...
                .andExpect(request -> {
                    ...
                    var verified = codec.verify(
                            ...
                            new JwtVerificationPolicy(
                                    ...
                                    keyId -> keys.getPublic()));
                    assertThat(verified.subject())
                            .isEqualTo(principal.accountId().toString());
        ...
        var response = client.exchange(new PropertyAdminClient.Request(
                HttpMethod.GET,
