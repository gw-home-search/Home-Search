...
public class AdminAccountService {
    ...
    public List<AccountSummary> accounts() {
        return jdbc.sql("""
            ...
            """).query((rs, row) -> new AccountSummary(rs.getObject("id", UUID.class), rs.getString("login_id"),
                rs.getString("display_name"), rs.getBoolean("enabled"), rs.getObject("locked_until", OffsetDateTime.class),
                Set.of((String[]) rs.getArray("roles").getArray()))).list();
    }
    ...
    public AccountSummary create(UUID actorId, CreateAccount command) {
        ...
        jdbc.sql("INSERT INTO admin.admin_account(id,login_id,display_name,password_hash) VALUES (:id,:loginId,:displayName,:hash)")
            .param("id", id).param("loginId", command.loginId()).param("displayName", command.displayName())
            .param("hash", passwordEncoder.encode(command.password())).update();
        command.roles().forEach(role -> jdbc.sql("INSERT INTO admin.admin_account_role(account_id,role_code) VALUES (:id,:role)")
            .param("id", id).param("role", role).update());
        audit(actorId, id, "ACCOUNT_CREATED");
        return new AccountSummary(id, command.loginId(), command.displayName(), true, null, Set.copyOf(command.roles()));
    }
    ...
    public void replaceRoles(UUID actorId, UUID accountId, Set<String> roles) {
        ...
        jdbc.sql("DELETE FROM admin.admin_account_role WHERE account_id=:id").param("id", accountId).update();
        roles.forEach(role -> jdbc.sql("INSERT INTO admin.admin_account_role(account_id,role_code) VALUES (:id,:role)")
            .param("id", accountId).param("role", role).update());
    ...
    public void setEnabled(UUID actorId, UUID accountId, boolean enabled) {
        ...
        int updated = jdbc.sql("UPDATE admin.admin_account SET enabled=:enabled,updated_at=now() WHERE id=:id")
            .param("enabled", enabled).param("id", accountId).update();
    ...
    private void revokeSessionsFor(UUID accountId) {
        jdbc.sql("DELETE FROM admin.spring_session WHERE principal_name=(SELECT login_id FROM admin.admin_account WHERE id=:id)")
            .param("id", accountId).update();
    }
    private void requireAccount(UUID id) {
        if (!jdbc.sql("SELECT EXISTS(SELECT 1 FROM admin.admin_account WHERE id=:id)").param("id", id).query(Boolean.class).single())
    ...
    private void ensureNotLastActiveAdmin(UUID accountId) {
        boolean targetIsActiveAdmin = jdbc.sql("""
        ...
        boolean anotherActiveAdmin = jdbc.sql("""
            SELECT EXISTS(SELECT 1 FROM admin.admin_account a JOIN admin.admin_account_role ar ON ar.account_id=a.id
                          WHERE a.id<>:id AND a.enabled AND ar.role_code='ADMIN')
            """).param("id", accountId).query(Boolean.class).single();
    ...
    private void audit(UUID actor, UUID target, String type) {
        jdbc.sql("INSERT INTO admin.admin_security_audit_event(actor_account_id,target_account_id,event_type,success) VALUES (:actor,:target,:type,true)")
            .param("actor", actor).param("target", target).param("type", type).update();
    }

    public record CreateAccount(String loginId, String displayName, String password, Set<String> roles) {}
    public record AccountSummary(UUID accountId, String loginId, String displayName, boolean enabled,
                                 OffsetDateTime lockedUntil, Set<String> roles) {}
