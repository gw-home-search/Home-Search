...
final class AdminMigrationRunner implements ApplicationRunner, ExitCodeGenerator {
    ...
    AdminMigrationRunner(DataSource dataSource) { this.dataSource = dataSource; }
    ...
    public void run(ApplicationArguments arguments) {
        try {
            ...
            Flyway flyway = Flyway.configure().dataSource(dataSource)
                ...
                .schemas("admin").defaultSchema("admin")
            ...
            System.err.println("admin migration failed: " + exception.getMessage()); exitCode = 1;
        }
    }
    ...
    private void migrate(Flyway flyway, Map<String, String> options) {
        ...
        Flyway configured = Flyway.configure().configuration(flyway.getConfiguration()).target(target).load();
    ...
    private static String required(Map<String, String> options, String key) {
        String value = options.get(key); if (value == null || value.isBlank()) throw new UsageException("missing --" + key); return value;
    }
