    ...
    async def plan_query(
        ...
        output = await self._structured_response(
            ...
            developer_prompt=(
                ...
                "Use comparison when the user asks to compare 2 to 4 named apartment "
                "complexes. Preserve their order in complexNames, "
                "set complexName to the first name, and never choose a winner or generate table "
                "values. Leave exclusiveAreaSquareMeters null when area is omitted so the server "
                "can keep a non-price comparison useful. Leave complexNames empty for every other capability. "
                ...
                "missing; preserve missing values as null so the server can apply safe defaults "
                "and still return useful candidates. Set "
                "complexName to the region text or a short recommendation-intent label. Do not "
                "multiply Korean supply-area shorthand by 3.3058 and present it as exclusive "
                "area. For 30평대, use 84.0 square meters only as the common comparable "
                "exclusive-area proxy; otherwise leave an uncertain exclusive area null. "
                ...
                "Set recommendationMode to BUDGET only when region, maximum budget, and exclusive "
                "area are all present. Set it to CRITERIA when measurable conditions can be "
                ...
                "minimumUnitCount. Choose only relevant metrics from the closed runtime catalog: "
                "ACADEMY, SCHOOL, TRANSIT, and SHOPPING. Interpret broad property preferences "
                "semantically: education or school-district intent may use SCHOOL and ACADEMY, "
                "while family convenience may use measurable active sources that directly apply. "
                "Never add CHILDCARE to recommendationCriteria. Put every selected metric in "
                "criteriaOrder, using the user's explicit priority when present and otherwise a "
                "neutral relevance order. Set "
        ...
        *,
        ...
        output = await self._structured_response(
            ...
            developer_prompt=(
                ...
                "Select only the supplied facts that directly answer the user's goal. "
                "Do not repeat a fact already carried by a structured artifact. "
                ...
                " For academy location facts, state only observed academy name, category, "
                ...
                "matches or describe the observed result count as an official registry count."
                ...
                "are locations, not attendance zones or school quality. Academy counts are observed "
                "locations, not official academy registration counts. Childcare observations never "
