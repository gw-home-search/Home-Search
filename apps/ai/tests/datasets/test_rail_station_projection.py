...
def test_rail_projection_preserves_nullable_row_provenance_date(
    dataset_repository: PostgresDatasetRepository,
    postgres_dsn: str,
) -> None:
    rows = [
        ["서울교통공사", "02", "2호선", "201", "시청", "서울 중구", 37.5657, 126.977, "", "2025-12-31"],
        ["서울교통공사", "02", "2호선", "202", "을지로입구", "서울 중구", 37.566, 126.982, "", ""],
    ]
    result = DatasetLifecycleService(
        dataset_repository,
        clock=lambda: datetime(2026, 1, 2, tzinfo=UTC),
    ...
    with psycopg.connect(postgres_dsn) as connection:
        ...
        dates = connection.execute(
            """
            SELECT occurrence_id, row_reference_date
            FROM reference_read.rail_station_occurrence
            ORDER BY occurrence_id
            """
        ).fetchall()
