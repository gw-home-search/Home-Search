...
public class PricePredictionUseCase {
	...
	public PricePredictionResult getOrSchedulePrediction(Long complexId) {
		...
		return featureRepository.findFeature(complexId, anchorMonth)
	...
	private PricePredictionResult getOrSchedulePrediction(PredictionFeature feature, YearMonth anchorMonth) {
		...
		return cacheRepository.find(key)
			.orElseGet(() -> schedulePrediction(feature, key));
	}
	...
	private void runPrediction(PredictionFeature feature, PredictionCacheKey key) {
		try {
			...
			cacheRepository.save(key, PricePredictionResult.ready(feature, properties, clientResult, now()), properties.readyTtl());
		} catch (RuntimeException ex) {
			cacheRepository.save(key, PricePredictionResult.failed(feature, properties, now(), failureMessage(ex)),
