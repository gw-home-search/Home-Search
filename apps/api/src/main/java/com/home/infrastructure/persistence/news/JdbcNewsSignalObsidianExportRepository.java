...
class JdbcNewsSignalObsidianExportRepository implements NewsSignalObsidianExportRepository {
	...
	JdbcNewsSignalObsidianExportRepository(JdbcClient jdbcClient, ObjectMapper objectMapper) {
	...
	private <T> T readJson(String value, TypeReference<T> typeReference) {
		try {
			...
			throw new IllegalStateException("Failed to deserialize news signal Obsidian export JSON", exception);
		}
	}
}
