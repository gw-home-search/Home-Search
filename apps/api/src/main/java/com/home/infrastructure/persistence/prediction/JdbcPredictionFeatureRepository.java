...
public class JdbcPredictionFeatureRepository implements PredictionFeatureRepository {
	...
	private void putCoreFeatures(Map<String, Object> numeric, BasisTradeRow basisTrade, YearMonth anchorMonth) {
		...
		put(numeric, "age_years", basisTrade.useDate() == null ? null : anchorMonth.getYear() - basisTrade.useDate().getYear());
	}
	...
	private void putExactPrevFeatures(
		...
		put(numeric, "wide_prev1_present_exact_missing",
	...
	private void putMonthlyAnchorFeatures(
		...
		MonthlyAggregate complexLag1 = findMonthlyAggregate("complex", complexId, targetAreaM2, sggCode,
			anchorStart.minusMonths(1), anchorStart);
		MonthlyAggregate complexLag3 = findMonthlyAggregate("complex", complexId, targetAreaM2, sggCode,
			anchorStart.minusMonths(3), anchorStart);
		MonthlyAggregate exactLag1 = findMonthlyAggregate("exact", complexId, targetAreaM2, sggCode,
			anchorStart.minusMonths(1), anchorStart);
		MonthlyAggregate exactLag3 = findMonthlyAggregate("exact", complexId, targetAreaM2, sggCode,
			anchorStart.minusMonths(3), anchorStart);
		MonthlyAggregate sggLag1 = findMonthlyAggregate("sgg", complexId, targetAreaM2, sggCode,
			anchorStart.minusMonths(1), anchorStart);
		MonthlyAggregate sggLag3 = findMonthlyAggregate("sgg", complexId, targetAreaM2, sggCode,
			anchorStart.minusMonths(3), anchorStart);
	...
	private MonthlyAggregate findMonthlyAggregate(
		...
		String scopePredicate = switch (scope) {
	...
	private void putCrossGaps(Map<String, Object> numeric) {
		put(numeric, "prev1_vs_complex_lag3m_log_gap", subtract(
			numeric.get("log_complex_prev_price_per_m2"),
			numeric.get("complex_lag3m_log_median_ppm")
		));
		put(numeric, "exact_prev1_vs_exact_lag3m_log_gap", subtract(
			numeric.get("log_exact_prev1_price_per_m2"),
			numeric.get("exact_area_lag3m_log_median_ppm")
		));
	}
	...
	private void putLogSummary(Map<String, Object> numeric, String prefix, List<TradeFeatureRow> rows) {
		...
		put(numeric, prefix + "_log_mean", logs.isEmpty() ? null : logs.stream().mapToDouble(Double::doubleValue).average().orElseThrow());
	...
	private Double logPricePerM2(TradeFeatureRow row) {
		if (row == null || row.dealAmount() == null || row.exclArea() == null || row.exclArea().signum() <= 0) {
			return null;
		}
	...
	private Double areaAbsDiff(BigDecimal targetAreaM2, TradeFeatureRow row) {
		return row == null || row.exclArea() == null ? null : targetAreaM2.subtract(row.exclArea()).abs().doubleValue();
	}
	...
	private record MonthlyAggregate(
		Double medianLogPpm,
		Double meanLogPpm,
		long count
	) {
