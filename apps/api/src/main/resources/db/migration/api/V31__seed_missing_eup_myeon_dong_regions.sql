-- Adds eup-myeon-dong regions confirmed in RTMS data but absent from the legacy nationwide seed,
-- then repairs parcel/complex region relations and rebuilds region unit count sums the same way
-- as V30 so the new regions immediately participate in marker aggregation.
...
LEFT JOIN LATERAL (
    ...
    FROM parcel p
    WHERE substring(p.pnu FROM 1 FOR 8) = seed.code
) center ON true
