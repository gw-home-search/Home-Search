...
class RedisPredictionCacheRepositoryTest {
	...
	void storesReadyPredictionWithCacheKeyAndTtl() {
		...
		verify(redis.operations()).set(
			org.mockito.ArgumentMatchers.eq("home-search:prediction:v1:F37:complex:501:basis:9001:ym:202606"),
			valueCaptor.capture(),
	...
	void acquiresLockWithLockKeyAndTtl() {
		RedisFixture redis = redis();
		when(redis.operations().setIfAbsent(anyString(), anyString(), any(Duration.class))).thenReturn(true);
		RedisPredictionCacheRepository repository = new RedisPredictionCacheRepository(redis.template(), objectMapper);

		boolean acquired = repository.acquireLock(
			new PredictionCacheKey(501L, 9001L, YearMonth.of(2026, 6)),
			Duration.ofSeconds(60)
		);

		assertThat(acquired).isTrue();
		verify(redis.operations()).setIfAbsent(
