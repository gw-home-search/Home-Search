...
class JdbcPredictionFeatureRepositoryTest extends JdbcPostgresTestSupport {
	...
	void predictionFeatureUsesLatestTradeBasisAndPnuEmbedding() {
		...
		assertThat(repository.findFeature(501L, YearMonth.of(2026, 6)))
			.hasValueSatisfying(feature -> {
				...
				assertThat(number(feature.numericFeatures().get("complex_prev_missing"))).isZero();
				assertThat(number(feature.numericFeatures().get("exact_prev1_missing"))).isZero();
				assertThat(number(feature.numericFeatures().get("exact_prev3_area_abs_diff"))).isCloseTo(
					0.37,
