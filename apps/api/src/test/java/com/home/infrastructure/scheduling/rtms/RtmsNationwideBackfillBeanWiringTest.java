	...
	private static void registerRtmsExternalThenIngestPersistence(GenericApplicationContext context) {
		try {
			...
			reader.register((Class<Object>) Class.forName(
				"com.home.infrastructure.external.rtms.RtmsExternalApiConfiguration"
			));
			reader.register(RtmsBatchOrchestrationConfiguration.class);
			reader.register((Class<Object>) Class.forName(
			...
			throw new IllegalStateException(
				"RtmsExternalApiConfiguration and IngestPersistenceConfiguration are required for RTMS wiring tests",
				ex
			);
		}
	}
