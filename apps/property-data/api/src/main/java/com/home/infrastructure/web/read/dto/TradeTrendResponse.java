...
public record TradeTrendResponse(String month, long avgAmount, int count, long minAmount, long maxAmount) {

    public static TradeTrendResponse from(TradeTrendPoint point) {
        return new TradeTrendResponse(
                point.month(), point.avgAmount(), point.count(), point.minAmount(), point.maxAmount());
    }
}
