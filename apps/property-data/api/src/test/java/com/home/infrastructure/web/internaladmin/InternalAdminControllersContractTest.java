...
class InternalAdminControllersContractTest {
    ...
    @MockitoBean CoordinateOverrideAdminService coordinateService;
    ...
    void coordinateOverrideDerivesActorFromPrincipalAndRejectsMissingPermission() throws Exception {
        ...
        ArgumentCaptor<CoordinateOverrideApprovalCommand> command = ArgumentCaptor.forClass(CoordinateOverrideApprovalCommand.class);
    ...
    void exposesEveryPermissionBoundReadHoldAndAliasOperation() throws Exception {
        ...
        mvc.perform(get("/internal/v1/admin/coordinates/pending").requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal))
            .andExpect(status().isOk());
        mvc.perform(get("/internal/v1/admin/coordinates/pending/summary").requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal))
            .andExpect(status().isOk());
        mvc.perform(get("/internal/v1/admin/metadata/pending").requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal))
            .andExpect(status().isOk());
        mvc.perform(get("/internal/v1/admin/metadata/pending/summary").requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal))
            .andExpect(status().isOk());
        mvc.perform(get("/internal/v1/admin/metadata/501").requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal))
            .andExpect(status().isOk());
        mvc.perform(get("/internal/v1/admin/metadata/pnu-aliases").requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal))
            .andExpect(status().isOk());
        mvc.perform(post("/internal/v1/admin/metadata/501/hold")
                .requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal)
                .contentType(MediaType.APPLICATION_JSON).content("{\"reason\":\"manual hold\"}"))
            .andExpect(status().isOk());
        mvc.perform(post("/internal/v1/admin/metadata/pnu-aliases")
                .requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"canonicalPrefix\":\"11110101\",\"sourcePrefix\":\"11110102\",\"reason\":\"boundary change\"}"))
            .andExpect(status().isOk());
        ...
        mvc.perform(post("/internal/v1/admin/metadata/pnu-aliases/41/disable")
                .requestAttr(InternalAdminPrincipal.REQUEST_ATTRIBUTE, principal)
                .contentType(MediaType.APPLICATION_JSON).content("{\"reason\":\"obsolete\"}"))
            .andExpect(status().isOk());
    }
