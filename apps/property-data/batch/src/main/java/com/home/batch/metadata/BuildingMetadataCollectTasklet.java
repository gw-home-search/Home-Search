...
class BuildingMetadataCollectTasklet implements Tasklet {
	...
	BuildingMetadataCollectTasklet(BuildingMetadataBatchService service, BuildingMetadataExecutionLock executionLock,
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
		...
		try (BuildingMetadataExecutionLock.Lock ignored = executionLock.acquire()) {
			service.collect(params.get("mode").toString(), maxRequests, optionalLong(params.get("fromComplexId")),
				optionalLong(params.get("toComplexId")), UUID.fromString(params.get("requestId").toString()));
		}
		return RepeatStatus.FINISHED;
	}

	private Long optionalLong(Object value) { return value == null ? null : Long.valueOf(value.toString()); }
}
