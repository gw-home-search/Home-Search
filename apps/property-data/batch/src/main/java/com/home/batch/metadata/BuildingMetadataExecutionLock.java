...
class BuildingMetadataExecutionLock {
	...
	BuildingMetadataExecutionLock(DataSource dataSource) { this.dataSource = Objects.requireNonNull(dataSource); }
	...
	private void release(Connection connection) {
		try {
			if (!execute(connection, RELEASE_SQL)) throw new IllegalStateException("building metadata advisory unlock failed");
		}
		catch (SQLException exception) {
	...
	@FunctionalInterface
	interface Lock extends AutoCloseable { @Override void close(); }
}
