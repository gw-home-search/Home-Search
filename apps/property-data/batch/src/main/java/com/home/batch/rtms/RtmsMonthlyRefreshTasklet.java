...
public class RtmsMonthlyRefreshTasklet implements Tasklet {
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
		ExecutionContext jobContext = chunkContext.getStepContext()
			...
			.getJobExecution()
			.getExecutionContext();
		List<RtmsRefreshWorkUnit> workset = workset(jobContext, contribution.getStepExecution().getJobParameters());
	...
	private List<RtmsRefreshWorkUnit> backfillWorkset(JobParameters parameters) {
		return planner.backfill(
			parameters.getString("fromYmd"),
			parameters.getString("toYmd"),
			parameters.getString("lawdCds")
		);
	}

	private static String serialize(List<RtmsRefreshWorkUnit> workset) {
		return String.join(";", workset.stream().map(RtmsRefreshWorkUnit::serialized).toList());
	}
}
