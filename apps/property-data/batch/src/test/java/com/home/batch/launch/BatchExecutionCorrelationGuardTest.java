...
class BatchExecutionCorrelationGuardTest {
	...
	void setUp() {
		DriverManagerDataSource dataSource = new DriverManagerDataSource(
			"jdbc:h2:mem:correlation-guard;MODE=PostgreSQL;DB_CLOSE_DELAY=-1;DATABASE_TO_LOWER=TRUE",
			"sa",
			""
		);
	...
	void rejectsReuseAcrossDifferentParameterSetOrJob() {
		...
		assertThatThrownBy(() -> guard.verify(
			"rtmsDailyRefreshJob",
			parameters("2026-07-11", REQUEST_ID)
		))
	...
	private void seedExecution(long instanceId, long executionId, String jobName, String runDate, String requestId) {
		jdbcClient.sql("INSERT INTO batch.BATCH_JOB_INSTANCE VALUES (:instanceId, :jobName)")
			.param("instanceId", instanceId)
			.param("jobName", jobName)
			.update();
		jdbcClient.sql("INSERT INTO batch.BATCH_JOB_EXECUTION VALUES (:executionId, :instanceId)")
