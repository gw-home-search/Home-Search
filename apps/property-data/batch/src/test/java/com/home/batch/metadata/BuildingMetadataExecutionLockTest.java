...
class BuildingMetadataExecutionLockTest {
    ...
    void holdsSameConnectionUntilUnlock() throws Exception {
        ...
        given(connection.prepareStatement("SELECT pg_try_advisory_lock(hashtext(?))"))
                .willReturn(acquire);
        given(connection.prepareStatement("SELECT pg_advisory_unlock(hashtext(?))"))
                .willReturn(release);
        given(acquire.executeQuery()).willReturn(acquired);
        given(release.executeQuery()).willReturn(released);
    ...
    void closesConnectionWhenLockIsBusy() throws Exception {
        ...
        assertThatThrownBy(() -> new BuildingMetadataExecutionLock(dataSource).acquire())
                ...
                .hasMessageContaining("another building metadata job");
        verify(connection).close();
    }
    ...
    private ResultSet result(boolean value) throws Exception {
        ...
        given(result.getBoolean(1)).willReturn(value);
        return result;
    }
}
