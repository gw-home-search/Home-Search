...
class RtmsMonthlyRefreshTaskletTest {
	...
	void taskletRunsPlannedWorksetSequentiallyAndStoresSnapshot() throws Exception {
		...
		when(useCase.refresh("11680", "202607", correlationId))
			.thenReturn(RtmsMonthlyRefreshRunSummary.completed("11680", "202607", 1, new IngestResult(1, 1, 1, 0, 0, 0), 1L));
		when(useCase.refresh("11680", "202606", correlationId))
			.thenReturn(RtmsMonthlyRefreshRunSummary.completed("11680", "202606", 1, new IngestResult(1, 1, 0, 1, 0, 0), 2L));
		RtmsMonthlyRefreshTasklet tasklet = new RtmsMonthlyRefreshTasklet(
			useCase,
			new RtmsRefreshWorksetPlanner(RegionSiGunGuCodeReader.empty()),
			"11680",
			1,
			true
		);
		StepExecution stepExecution = stepExecution(new JobParameters(
			java.util.Map.of(
		...
		assertThat(stepExecution.getJobExecution().getExecutionContext()
