...
public class ComplexMatchCandidatePolicy {
    ...
    public ComplexMatchResult match(
        ...
        List<ComplexMatchCandidate> aptSeqMatches =
                List.copyOf(Objects.requireNonNullElse(aptSeqCandidates, List.of()));
    ...
    private ComplexMatchResult matchUniqueAptSeq(
        ...
        if (!jibunPnu.derivedPnu().equals(candidate.parcelPnu())) {
            ...
            return ComplexMatchResult.failed(
                    ...
                    "APTSEQ parcel pnu conflict aptSeq=%s derivedPnu=%s complexPnu=%s"
                            .formatted(aptSeq, jibunPnu.derivedPnu(), candidate.parcelPnu()),
                    jibunPnu,
                    conflictCandidateCount(candidate, pnuCandidates),
                    candidateIds);
        }
        TradeMatchStatus status =
                nameVariant(aptName, candidate) ? TradeMatchStatus.MATCHED_NAME_VARIANT : TradeMatchStatus.MATCHED;
        String failureReason = status.isMatchedNameVariant() ? "observed RTMS name differs from master name" : null;
        return matched(
                candidate, TradeMatchPath.APTSEQ, status, jibunPnu, 1, List.of(candidate.complexId()), failureReason);
    }
    ...
    private int conflictCandidateCount(
            ComplexMatchCandidate aptSeqCandidate, List<ComplexMatchCandidate> pnuCandidates) {
        return allConflictCandidateIds(aptSeqCandidate, pnuCandidates).size();
    }

    private List<Long> conflictCandidateIds(
            ComplexMatchCandidate aptSeqCandidate, List<ComplexMatchCandidate> pnuCandidates) {
    ...
    private record CandidateNameMatch(ComplexMatchCandidate candidate, TradeMatchPath matchPath) {}

    private record NameScore(int value, TradeMatchPath matchPath) {}
}
