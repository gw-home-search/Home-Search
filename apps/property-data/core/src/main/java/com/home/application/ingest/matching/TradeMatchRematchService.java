...
public class TradeMatchRematchService {
    ...
    public TradeMatchRematchResult rematchHeld(int limit) {
        ...
        for (RawTradeIngestRecord raw :
                rawTradeIngestRepository.findByStatus(RawTradeIngestStatus.MATCH_FAILED, limit)) {
    ...
    private TradeMatchRematchResult rematchOne(RawTradeIngestRecord raw, TradeMatchRematchResult result) {
        ...
        if (item == null) {
            rawTradeIngestRepository.updateStatus(
                    raw.id(), RawTradeIngestTransition.parseFailed("rematch raw payload cannot be parsed"));
        ...
        try {
            ...
            rawTradeIngestRepository.updateStatus(
                    raw.id(), RawTradeIngestTransition.parseFailed(exception.getMessage()));
            return result.plusParseFailed();
        }
        ...
        if (match == null || !match.matched()) {
            rawTradeIngestRepository.updateStatus(
                    raw.id(), RawTradeIngestTransition.matchFailed(rematchFailureReason(match, bootstrapResult)));
