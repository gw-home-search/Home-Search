...
public class ComplexMetadataEnrichmentService {
    ...
    private ComplexMetadataResolution enrichOne(ComplexMetadataLookup lookup) {
        try {
            ...
            if (resolution == null) {
                resolution =
                        ComplexMetadataResolution.unavailable(null, "complex metadata resolver returned no result");
            }
            repository.saveResolution(lookup.complexId(), resolution, nextAttemptAt(lookup, resolution));
            return resolution;
        } catch (RuntimeException exception) {
            ComplexMetadataResolution failed =
                    ComplexMetadataResolution.failed(null, redactSensitive(exception.getMessage()));
    ...
    private void markFailed(
            ComplexMetadataLookup lookup, ComplexMetadataResolution failed, RuntimeException exception) {
        try {
    ...
    private Instant nextAttemptAt(ComplexMetadataLookup lookup, ComplexMetadataResolution resolution) {
        ...
        return retryPolicy
                .nextAttemptAt(resolution.status(), resolution.failureKind(), attemptNo, Instant.now())
                .orElse(null);
    }
