...
public class ComplexMetadataResolutionPolicy {
    ...
    public ComplexMetadataResolution resolve(
        ...
        if (hasMetadata(odcloud)) {
            ...
            ComplexMetadataResolution building = Objects.requireNonNull(
                            buildingResolver, "buildingResolver is required")
                    .get();
            if (hasMetadata(building)) {
                if (conflicts(odcloud.metadata(), building.metadata())) {
                    return ComplexMetadataResolution.ambiguous("ODC+BLD", "complex metadata source conflict pnu=" + pnu)
        ...
        return Objects.requireNonNull(buildingResolver, "buildingResolver is required")
                .get();
    }
