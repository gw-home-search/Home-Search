...
public class ComplexMetadataRetryPolicy {
    ...
    private static final Duration[] PARTIAL_BACKOFF = {Duration.ofDays(14), Duration.ofDays(30), Duration.ofDays(90)};
    private static final Duration[] SOURCE_MISSING_BACKOFF = {Duration.ofDays(30), Duration.ofDays(90)};
    ...
    private static final Duration[] TRANSIENT_FAILURE_BACKOFF = {
        Duration.ofDays(1), Duration.ofDays(3), Duration.ofDays(7), Duration.ofDays(30)
    };

    public Optional<Instant> nextAttemptAt(
            ComplexMetadataStatus status, ComplexMetadataFailureKind failureKind, int attemptNo, Instant now) {
    ...
    private Optional<Instant> unavailableNextAttempt(
            ComplexMetadataFailureKind failureKind, int attemptNo, Instant now) {
    ...
    private Optional<Instant> failedNextAttempt(ComplexMetadataFailureKind failureKind, int attemptNo, Instant now) {
