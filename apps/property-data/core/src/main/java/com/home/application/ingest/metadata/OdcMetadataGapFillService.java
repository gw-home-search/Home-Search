...
public class OdcMetadataGapFillService {
    ...
    public OdcMetadataGapFillSummary fill(int maxTargets, Long fromComplexId, long toComplexId, UUID requestId) {
        ...
        for (OdcMetadataGapFillTarget target :
                repository.findTargets(maxTargets, fromComplexId, toComplexId, requestId)) {
            targets++;
            ...
            if (target.pnuComplexCount() != 1) {
                ...
                try {
                    ...
                    resolution = ComplexMetadataResolution.failed(
                            "ODC", ComplexMetadataFailureKind.TRANSIENT, redact(exception.getMessage()));
                }
