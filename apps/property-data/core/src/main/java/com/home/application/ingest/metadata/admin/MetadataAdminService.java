...
public class MetadataAdminService {
    ...
    public Detail detail(long complexId) {
        return repository.detail(complexId);
    }

    public ActionResult retry(long complexId, String actor, String reason) {
        validateDecision(actor, reason);
        return repository.retry(complexId, actor.trim(), reason.trim());
    }

    public ActionResult hold(long complexId, String actor, String reason) {
        validateDecision(actor, reason);
        return repository.hold(complexId, actor.trim(), reason.trim());
    }
    ...
    public Alias proposeAlias(String canonicalPrefix, String sourcePrefix, String actor, String reason) {
        validatePrefix(canonicalPrefix);
        validatePrefix(sourcePrefix);
        validateDecision(actor, reason);
        if (canonicalPrefix.equals(sourcePrefix))
            throw new InvalidMetadataAdminRequestException("prefixes must differ");
    ...
    public ActionResult approveAlias(long aliasId, String actor, String reason) {
        validateDecision(actor, reason);
        return repository.approveAlias(aliasId, actor.trim(), reason.trim());
    }

    public ActionResult disableAlias(long aliasId, String actor, String reason) {
        validateDecision(actor, reason);
        return repository.disableAlias(aliasId, actor.trim(), reason.trim());
    }
    ...
    private void validateDecision(String actor, String reason) {
        if (actor == null
                ...
                || reason.isBlank()
                || reason.length() > 1000)
            throw new InvalidMetadataAdminRequestException("actor and reason are required");
    }

    private void validatePrefix(String prefix) {
        if (prefix == null || !prefix.matches("\\d{8}"))
            throw new InvalidMetadataAdminRequestException("prefix must be 8 digits");
    }
}
