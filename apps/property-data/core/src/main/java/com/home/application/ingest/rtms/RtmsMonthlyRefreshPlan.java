...
public record RtmsMonthlyRefreshPlan(String lawdCd, String baseDealYmd, int lookbackMonths) {

    private static final DateTimeFormatter FORMATTER = DateTimeFormatter.ofPattern("yyyyMM");
    private static final int MAX_LOOKBACK_MONTHS = 24;

    public RtmsMonthlyRefreshPlan {
    ...
    public List<RtmsApartmentTradeRequest> monthlyRequests() {
        YearMonth baseMonth = YearMonth.parse(baseDealYmd, FORMATTER);
        return IntStream.rangeClosed(0, lookbackMonths)
                .mapToObj(offset -> new RtmsApartmentTradeRequest(
                        lawdCd, baseMonth.minusMonths(offset).format(FORMATTER), 1))
                .toList();
    }

    public List<String> dealYmds() {
