...
import com.home.domain.insight.MarketInsightCoverageDecision;
...
@Service
public class MarketInsightDailyBuildService {

    private final MarketInsightBuildRepository repository;
    private final Clock clock;

    public MarketInsightDailyBuildService(MarketInsightBuildRepository repository) {
    ...
    public MarketInsightBuildResult build(LocalDate runDate) {
        ...
        if (source == null) {
            var snapshotId = repository.rejectDailyNationwide(
        ...
        MarketInsightCoverageDecision decision = MarketInsightCoveragePolicy.evaluate(source.coverage());
        if (!decision.publishable()) {
            var snapshotId = repository.rejectDailyNationwide(runDate, source, decision.reason(), clock.instant());
            return MarketInsightBuildResult.rejected(snapshotId, decision.reason());
        }
        return MarketInsightBuildResult.published(repository.publishDailyNationwide(source, clock.instant()));
    }
}
