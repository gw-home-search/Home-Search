...
public final class BuildingMetadataMatchPolicy {
	...
	public MatchResult resolveOdc(String pnu, List<InternalCandidate> internal, List<SourceCandidate> source) {
		...
		if (pnuInternal.size() == 1 && source.size() == 1) {
			...
			if (candidate.names().stream().allMatch(name -> ComplexNameNormalizer.normalize(name).isEmpty())) {
	...
	public MatchResult resolveBuilding(String pnu, List<InternalCandidate> internal, List<SourceCandidate> source) {
		...
		if (pnuInternal.size() == 1 && source.size() == 1) {
			...
			SourceCandidate candidate = source.get(0);
			String name = candidate.names().isEmpty() ? "" : ComplexNameNormalizer.normalize(candidate.names().get(0));
			if (!name.isEmpty() && bestScore(target, candidate) == 0) {
				return conflict("building register name conflicts with the PNU complex");
			}
			return resolved(target.complexId(), candidate, name.isEmpty() ? "BLD_SINGLE_PNU_EMPTY_NAME" : "BLD_EXACT_NAME");
		}
		MatchResult result = mutualUnique(pnuInternal, source, "BLD_EXACT_NAME");
		if (result.status().isResolvedLike()) {
			InternalCandidate target = pnuInternal.stream().filter(item -> item.complexId() == result.complexId()).findFirst().orElseThrow();
	...
	private MatchResult mutualUnique(List<InternalCandidate> internal, List<SourceCandidate> source, String path) {
		...
		List<Pair> mutual = pairs.stream().filter(pair -> {
			List<Pair> forTarget = pairs.stream().filter(other -> other.internal().complexId() == pair.internal().complexId())
				.sorted(Comparator.comparingInt(Pair::score).reversed()).toList();
			return forTarget.size() == 1 || forTarget.get(0).score() > forTarget.get(1).score();
		}).toList();
	...
	private boolean auxiliaryConflict(BuildingMetadataValues current, BuildingMetadataValues candidate) {
		...
		return different(left.dongCnt(), right.dongCnt()) || different(left.unitCnt(), right.unitCnt())
			|| different(left.useDate(), right.useDate());
	}

	private boolean different(Object left, Object right) { return left != null && right != null && !left.equals(right); }
	private List<InternalCandidate> pnuInternal(String pnu, List<InternalCandidate> internal) {
		return internal == null ? List.of() : internal.stream().filter(item -> pnu.equals(item.pnu())).toList();
	}
	private void validatePnu(String pnu) { if (pnu == null || !pnu.matches("\\d{19}")) throw new IllegalArgumentException("PNU must be 19 digits"); }
	private MatchResult resolved(long id, SourceCandidate candidate, String path) { return new MatchResult(ComplexMetadataStatus.RESOLVED, id, path, null, null, candidate); }
	private MatchResult unavailable() { return new MatchResult(ComplexMetadataStatus.UNAVAILABLE, null, null, ComplexMetadataFailureKind.SOURCE_MISSING, "source candidate unavailable", null); }
	private MatchResult conflict(String reason) { return new MatchResult(ComplexMetadataStatus.AMBIGUOUS, null, null, ComplexMetadataFailureKind.AMBIGUOUS, reason, null); }
	private MatchResult ambiguous(String reason) { return new MatchResult(ComplexMetadataStatus.AMBIGUOUS, null, null, ComplexMetadataFailureKind.AMBIGUOUS, reason, null); }

	public record InternalName(String type, String value, int priority) {
		public InternalName { Objects.requireNonNull(type); }
	}
	public record InternalCandidate(long complexId, String pnu, List<InternalName> names, BuildingMetadataValues values) {
		public InternalCandidate { names = names == null ? List.of() : List.copyOf(names); }
	}
	public record SourceCandidate(String sourceKey, String pnu, List<String> names, BuildingMetadataValues values, Long identityComplexId) {
		public SourceCandidate { names = names == null ? List.of() : List.copyOf(names); }
	}
	public record MatchResult(ComplexMetadataStatus status, Long complexId, String matchPath,
		ComplexMetadataFailureKind failureKind, String failureReason, SourceCandidate candidate) {}
