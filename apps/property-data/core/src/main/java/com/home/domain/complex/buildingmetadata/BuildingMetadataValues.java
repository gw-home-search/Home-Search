...
public record BuildingMetadataValues(
	...
	public BuildingMetadataValues sanitized() {
		return new BuildingMetadataValues(
			positive(dongCnt), positive(unitCnt), positive(platArea), positive(archArea),
			positive(totArea), positive(bcRat), positive(vlRat), useDate
		);
	}

	public boolean hasAnyValue() {
		BuildingMetadataValues value = sanitized();
		return value.dongCnt != null || value.unitCnt != null || value.platArea != null || value.archArea != null
			|| value.totArea != null || value.bcRat != null || value.vlRat != null || value.useDate != null;
	}

	public boolean hasAllAreaValues() {
		BuildingMetadataValues value = sanitized();
		return value.platArea != null && value.archArea != null && value.totArea != null
			&& value.bcRat != null && value.vlRat != null;
	}
