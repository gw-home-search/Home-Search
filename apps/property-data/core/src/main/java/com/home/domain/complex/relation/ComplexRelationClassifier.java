...
public class ComplexRelationClassifier {
    ...
    public ComplexRelationClassifier(
            int redevelopmentGapMonths, int minTradeCountForRedevelopment, int useDateGapYears) {
    ...
    public ComplexRelationClassification classify(List<ComplexTradeSpan> input) {
        ...
        if (spans.size() == 1) {
            return new ComplexRelationClassification(
                    ComplexRelationType.SINGLE, spans, "complex count is 1", ComplexRelationConfidence.HIGH);
        }
        List<ComplexTradeSpan> tradeSpans =
                spans.stream().filter(ComplexTradeSpan::hasTradeSpan).toList();
    ...
    private List<ComplexTradeSpan> sorted(List<ComplexTradeSpan> input) {
        return (input == null ? List.<ComplexTradeSpan>of() : input)
                .stream()
                        .sorted(Comparator.comparing(
                                        ComplexTradeSpan::firstDeal, Comparator.nullsLast(Comparator.naturalOrder()))
                                .thenComparing(
                                        ComplexTradeSpan::complexId, Comparator.nullsLast(Comparator.naturalOrder())))
                        .toList();
    }
    ...
    private boolean overlaps(ComplexTradeSpan left, ComplexTradeSpan right) {
        return !left.lastDeal().isBefore(right.firstDeal()) && !right.lastDeal().isBefore(left.firstDeal());
    }
    ...
    private boolean hasEnoughTradeSamplesForRedevelopment(List<ComplexTradeSpan> spans) {
        return spans.stream().allMatch(span -> span.tradeCount() >= minTradeCountForRedevelopment);
    }
    ...
    private ComplexRelationClassification unknown(List<ComplexTradeSpan> spans, String reason) {
        return new ComplexRelationClassification(
                ComplexRelationType.UNKNOWN, spans, reason, ComplexRelationConfidence.NONE);
    }
}
