 ...
 */
public record RawTradeIngestTransition(RawTradeIngestStatus status, String failureReason) {
    ...
    public static RawTradeIngestTransition sourceKeyDuplicate() {
        return new RawTradeIngestTransition(
                RawTradeIngestStatus.DUPLICATE, RawTradeIngestFailureReason.SOURCE_KEY_DUPLICATE.value());
    }

    public static RawTradeIngestTransition fallbackIdentityDuplicate() {
        return new RawTradeIngestTransition(
                RawTradeIngestStatus.DUPLICATE, RawTradeIngestFailureReason.FALLBACK_IDENTITY_DUPLICATE.value());
    }

    public static RawTradeIngestTransition canceledSourceKey() {
        return new RawTradeIngestTransition(
                RawTradeIngestStatus.CANCELED, RawTradeIngestFailureReason.CANCELED_SOURCE_KEY.value());
    }

    public static RawTradeIngestTransition rematchSourceKeyDuplicate() {
        return new RawTradeIngestTransition(
                RawTradeIngestStatus.DUPLICATE, RawTradeIngestFailureReason.REMATCH_SOURCE_KEY_DUPLICATE.value());
    }
