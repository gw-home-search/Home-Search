...
public final class RedisNearbyPlaceCache implements NearbyPlaceCache {
	...
	private boolean valid(NearbyPlaceProviderResult result, NearbyPlaceCacheKey key) {
		if (result == null || result.category() != key.category() || result.matchedCount() < 0
			|| result.places() == null || result.places().size() > 15 || result.retrievedAt() == null) {
			return false;
		}
		return result.places().stream().allMatch(place -> {
			if (place == null || place.placeId() == null || !place.placeId().startsWith("kakao:")
				|| place.name() == null || place.name().isBlank()
				|| !Double.isFinite(place.lat()) || place.lat() < -90 || place.lat() > 90
				|| !Double.isFinite(place.lng()) || place.lng() < -180 || place.lng() > 180
	...
	private void record(String result) {
		meterRegistry.counter("home.search.nearby.place.cache.requests", "result", result).increment();
	}
}
