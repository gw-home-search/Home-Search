...
public class ComplexMetadataClientConfiguration {
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ComplexMetadataClientConfiguration.class);
	...
	BuildingMetadataSourceClient buildingMetadataSourceClient(
		...
		@Value("${apis.data.bld-title-path:/1613000/BldRgstHubService/getBrRecapTitleInfo}") String legacyBldTitlePath,
		@Value("${apis.data.recap-title-path:/1613000/BldRgstHubService/getBrTitleInfo}") String legacyRecapTitlePath,
		...
		BuildingMetadataEndpointPaths paths = BuildingMetadataEndpointPaths.resolve(canonicalTitlePath,
			canonicalRecapPath, legacyBldTitlePath, legacyRecapTitlePath);
		warnLegacy(paths);
		return new PublicBuildingMetadataSourceClient(
			...
			paths.recap(), paths.title(), minRequestIntervalMillis
		);
	}
	...
	private void warnLegacy(BuildingMetadataEndpointPaths paths) {
		if (paths.usesLegacy()) {
			log.warn("legacy building metadata endpoint properties are in use; migrate to apis.data.building-title-path and apis.data.building-recap-title-path");
		}
	}
