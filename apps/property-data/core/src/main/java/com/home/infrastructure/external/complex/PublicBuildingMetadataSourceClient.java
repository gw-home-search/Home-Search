...
public class PublicBuildingMetadataSourceClient implements BuildingMetadataSourceClient {
	...
	public PublicBuildingMetadataSourceClient(RestClient odcClient, String odcBaseUrl, String odcServiceKey,
		String odcPath, RestClient buildingClient, String buildingBaseUrl, String buildingServiceKey,
		String recapPath, String titlePath, long minIntervalMillis) {
	...
	public BuildingMetadataSourceResponse fetch(BuildingMetadataSourceKind sourceKind, String pnu) {
		...
		return switch (sourceKind) {
			...
			case BLD_RECAP_TITLE -> exchange(buildingClient, buildingBaseUrl, recapPath, buildingQuery(pnu), sourceKind, pnu);
			case BLD_TITLE -> exchange(buildingClient, buildingBaseUrl, titlePath, buildingQuery(pnu), sourceKind, pnu);
		};
	}

	private BuildingMetadataSourceResponse exchange(RestClient client, String baseUrl, String path, String query,
		BuildingMetadataSourceKind kind, String pnu) {
		return client.get().uri(ExternalApiUri.create(baseUrl, path, query)).exchange((request, response) -> {
			try {
				...
				return new BuildingMetadataSourceResponse(kind, pnu, response.getStatusCode().value(), null,
					body.body(), body.byteSize(), body.hash(), body.oversized());
			}
	...
	private synchronized void throttle() {
		...
		if (remaining > 0) {
			...
			catch (InterruptedException exception) { Thread.currentThread().interrupt(); throw new IllegalStateException("metadata request interrupted", exception); }
		}
