...
class RequiredRtmsCoordinateSourcePreflight implements RtmsCoordinateSourcePreflight {
    ...
    RequiredRtmsCoordinateSourcePreflight(
            boolean allowCoordinatePendingOnly, RtmsCoordinateSourceAvailabilityProbe availabilityProbe) {
    ...
    public void verify() {
        if (allowCoordinatePendingOnly) {
            log.warn("RTMS coordinate source preflight bypassed; coordinate-pending parcels can be stored but public "
                    + "map markers will not appear until approved coordinates are supplied");
            return;
        }
        if (!availabilityProbe.configured()) {
