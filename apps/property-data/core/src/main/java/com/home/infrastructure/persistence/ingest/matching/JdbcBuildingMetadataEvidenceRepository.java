...
public class JdbcBuildingMetadataEvidenceRepository implements BuildingMetadataEvidenceRepository {
    ...
    public BuildingMetadataAttemptResult recordAmbiguousPnu(BuildingMetadataTarget target, UUID requestId) {
        ...
        return transaction.execute(status -> record(
                target,
                source,
                ComplexMetadataStatus.AMBIGUOUS,
                ComplexMetadataFailureKind.AMBIGUOUS,
                "multiple complexes share the requested PNU",
                null,
                target.pnuComplexCount(),
                false,
                requestId,
                null));
    }
    ...
    @Override
    public BuildingMetadataAttemptResult apply(
    ...
    @Override
    public BuildingMetadataAttemptResult recordFailure(
            BuildingMetadataTarget target,
            BuildingMetadataSourceKind source,
            ComplexMetadataStatus status,
            ComplexMetadataFailureKind kind,
            String reason,
            UUID requestId,
            Instant nextAt) {
        return transaction.execute(
                tx -> record(target, source, status, kind, reason, null, null, false, requestId, nextAt));
    }

    private BuildingMetadataAttemptResult applyLocked(
            BuildingMetadataTarget target,
            BuildingMetadataSourceKind source,
            ParsedBuildingMetadataSource parsed,
            UUID requestId) {
        ...
        if (!locked.pnu().equals(target.pnu()) || pnuCount(locked.pnu()) != 1) {
            return record(
                    ...
                    ComplexMetadataStatus.AMBIGUOUS,
                    ComplexMetadataFailureKind.AMBIGUOUS,
                    "PNU changed or is no longer unique",
                    null,
                    parsed.candidates().size(),
                    false,
                    requestId,
                    null);
        }
        if (parsed.candidates().size() != 1) {
            return record(
                    ...
                    ComplexMetadataStatus.AMBIGUOUS,
                    ComplexMetadataFailureKind.AMBIGUOUS,
                    "building source returned multiple candidates",
                    null,
                    parsed.candidates().size(),
                    false,
                    requestId,
        ...
        if (candidate.sourceKey() == null || candidate.sourceKey().isBlank()) {
            return record(
                    ...
                    "building register management key is missing",
                    null,
                    1,
                    false,
                    requestId,
        ...
        Long owner = jdbc.sql("SELECT id FROM complex WHERE bld_mgm_bld_rgst_pk=:key")
                .param("key", candidate.sourceKey())
                .query(Long.class)
                .optional()
                .orElse(null);
        if (owner != null && owner != target.complexId())
            return conflict(target, source, "building register identity belongs to another complex", requestId);

        InternalCandidate internal = new InternalCandidate(
                target.complexId(), locked.pnu(), names(target.complexId(), locked), locked.values());
        MatchResult match = matchPolicy.resolveBuilding(locked.pnu(), List.of(internal), List.of(candidate));
        if (!match.status().isResolvedLike())
            return conflict(
                    target,
                    source,
                    match.failureReason() == null ? "building candidate identity is ambiguous" : match.failureReason(),
                    requestId);
        ProjectionDecision projection = projectionPolicy.decide(locked.values(), candidate.values());
        if (!projection.apply())
            return conflict(target, source, "existing metadata conflicts with building source", requestId);
        ...
        int updated = jdbc.sql("""
			...
			""")
                .param("dong", merged.dongCnt())
                .param("unit", merged.unitCnt())
                .param("plat", merged.platArea())
                .param("arch", merged.archArea())
                .param("tot", merged.totArea())
                .param("bc", merged.bcRat())
                .param("vl", merged.vlRat())
                .param("use_date", merged.useDate())
                .param("key", candidate.sourceKey())
                .param("metadata_status", metadataStatus)
                .param("source", source.name())
                .param("changed", changed)
                .param("id", target.complexId())
                .update();
        if (updated != 1) return conflict(target, source, "building register identity changed concurrently", requestId);
        String alias = candidate.names().stream()
    ...
    private BuildingMetadataAttemptResult conflict(
            BuildingMetadataTarget target, BuildingMetadataSourceKind source, String reason, UUID requestId) {
        return record(
                target,
                source,
                ComplexMetadataStatus.AMBIGUOUS,
    ...
    private BuildingMetadataAttemptResult record(
            BuildingMetadataTarget target,
            BuildingMetadataSourceKind source,
            ComplexMetadataStatus status,
            ComplexMetadataFailureKind kind,
        ...
        jdbc.sql("""
			...
			""")
                ...
                .param("no", attemptNo)
                .param("status", status.name())
                .param("source", source == null ? "BLD_RECAP_TITLE" : source.name())
                .param("kind", kind == null ? null : kind.name())
                .param("reason", reason)
                .param("next_at", offset(nextAt))
                .param("pnu", locked.pnu())
                .param("resolved", resolvedPnu)
                .param("count", candidates)
                .param("request_id", requestId)
    ...
    private List<InternalName> names(long id, LockedComplex locked) {
        List<InternalName> aliases = jdbc.sql("""
			...
			""")
                .param("id", id)
                .query((rs, n) -> new InternalName(rs.getString(1), rs.getString(2), 1))
                .list();
    ...
    private void upsertAlias(long id, String alias) {
        jdbc.sql("""
		...
		""")
                .param("id", id)
                .param("name", alias)
                .param("normalized", ComplexNameNormalizer.normalize(alias))
                .update();
    }

    private boolean coreComplete(BuildingMetadataValues value) {
        return value.dongCnt() != null && value.unitCnt() != null && value.useDate() != null;
    }
    ...
    private BuildingMetadataTarget target(ResultSet rs, int row) throws SQLException {
        return new BuildingMetadataTarget(
                rs.getLong("id"),
                rs.getString("pnu"),
                rs.getInt("pnu_complex_count"),
                rs.getString("bld_mgm_bld_rgst_pk"),
                values(rs));
    }

    private LockedComplex locked(ResultSet rs, int row) throws SQLException {
        return new LockedComplex(
                rs.getLong("id"),
                rs.getString("pnu"),
                rs.getString("name"),
    ...
    private BuildingMetadataValues values(ResultSet rs) throws SQLException {
        return new BuildingMetadataValues(
                (Integer) rs.getObject("dong_cnt"),
                (Integer) rs.getObject("unit_cnt"),
                rs.getBigDecimal("plat_area"),
                rs.getBigDecimal("arch_area"),
                rs.getBigDecimal("tot_area"),
                rs.getBigDecimal("bc_rat"),
                rs.getBigDecimal("vl_rat"),
                rs.getObject("use_date", LocalDate.class));
    }

    private record LockedComplex(
            long id, String pnu, String name, String tradeName, String key, BuildingMetadataValues values) {}
}
