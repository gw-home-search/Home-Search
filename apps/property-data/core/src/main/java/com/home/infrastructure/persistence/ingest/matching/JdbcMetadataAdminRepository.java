...
public class JdbcMetadataAdminRepository implements MetadataAdminRepository {
    ...
    public Summary summary() {
        List<Map.Entry<String, Long>> rows = jdbcClient
                .sql("""
			...
			""")
                .query((rs, n) -> Map.entry(rs.getString("metadata_status"), rs.getLong("count")))
                .list();
        Map<String, Long> counts =
                rows.stream().collect(java.util.stream.Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        return new Summary(counts.values().stream().mapToLong(Long::longValue).sum(), counts);
    }

    @Override
    public Detail detail(long complexId) {
        Pending complex = jdbcClient
                .sql("""
			SELECT c.id, COALESCE(c.trade_name, c.name) apt_name, c.apt_seq, p.pnu, p.address,
			       c.metadata_status, c.metadata_failure_kind, c.metadata_failure_reason, c.metadata_attempts,
			       c.metadata_next_attempt_at, c.metadata_hold_at, c.metadata_hold_reason
        ...
        List<Attempt> attempts = jdbcClient
                .sql("""
			...
			""")
                .param("id", complexId)
                .query((rs, n) -> new Attempt(
                        rs.getInt("attempt_no"),
                        rs.getString("status"),
                        rs.getString("source"),
                        rs.getString("failure_kind"),
                        rs.getString("failure_reason"),
                        rs.getString("lookup_path"),
                        rs.getString("requested_pnu"),
                        rs.getString("resolved_source_pnu"),
                        rs.getObject("alias_id", Long.class),
                        rs.getObject("candidate_count", Integer.class),
                        rs.getObject("next_attempt_at", java.time.OffsetDateTime.class),
                        rs.getObject("observed_at", java.time.OffsetDateTime.class)))
                .list();
        List<Decision> decisions = jdbcClient
                .sql("""
    ...
    @Override
    public ActionResult retry(long id, String actor, String reason) {
    ...
    @Override
    public ActionResult hold(long id, String actor, String reason) {
    ...
    @Override
    public Alias proposeAlias(String canonical, String source, String actor, String reason) {
        return transactionTemplate.execute(status -> {
            Alias alias = jdbcClient
                    .sql("""
    ...
    @Override
    public ActionResult approveAlias(long id, String actor, String reason) {
        return aliasDecision(id, actor, reason, "ALIAS_APPROVED", "APPROVED");
    }

    @Override
    public ActionResult disableAlias(long id, String actor, String reason) {
    ...
    private ActionResult aliasDecision(long id, String actor, String reason, String type, String target) {
        Boolean updated = transactionTemplate.execute(status -> {
            ...
            String extra = target.equals("APPROVED")
                    ? "approved_by=:actor,approved_at=now()"
                    : "disabled_by=:actor,disabled_at=now()";
            String allowedStatus = target.equals("APPROVED") ? "status='PENDING'" : "status IN ('PENDING','APPROVED')";
            int count = jdbcClient
                    .sql("UPDATE odcloud_pnu_prefix_alias SET status=:target, " + extra
    ...
    private void insertDecision(String targetColumn, long targetId, String type, String actor, String reason) {
        jdbcClient
                .sql("INSERT INTO complex_metadata_admin_decision(" + targetColumn
                        + ",decision_type,actor,reason) VALUES(:id,:type,:actor,:reason)")
                .param("id", targetId)
                .param("type", type)
                .param("actor", actor)
                .param("reason", reason)
    ...
    private Pending pending(ResultSet rs, int n) throws SQLException {
        return new Pending(
                ...
                rs.getString("metadata_failure_kind"),
                rs.getString("metadata_failure_reason"),
                rs.getInt("metadata_attempts"),
                rs.getObject("metadata_next_attempt_at", java.time.OffsetDateTime.class),
                rs.getObject("metadata_hold_at", java.time.OffsetDateTime.class),
    ...
    private Alias alias(ResultSet rs, int n) throws SQLException {
        return new Alias(
                rs.getLong("id"),
                rs.getString("canonical_prefix").trim(),
                rs.getString("source_prefix").trim(),
                OdcloudPnuAliasStatus.valueOf(rs.getString("status")),
                rs.getString("reason"),
