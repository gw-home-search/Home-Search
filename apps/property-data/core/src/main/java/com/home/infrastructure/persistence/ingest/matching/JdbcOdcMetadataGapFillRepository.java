...
import com.home.application.ingest.metadata.ComplexMetadataRetryPolicy;
...
public class JdbcOdcMetadataGapFillRepository implements OdcMetadataGapFillRepository {
	...
	public List<OdcMetadataGapFillTarget> findTargets(int limit, Long fromId, long toId, UUID requestId) {
		return jdbc.sql("""
			...
			""").param("from_id", fromId).param("to_id", toId)
	...
	public OdcMetadataGapFillOutcome recordAmbiguous(OdcMetadataGapFillTarget target, UUID requestId) {
		return transaction.execute(status -> {
			...
			return record(locked, ComplexMetadataResolution.ambiguous("ODC",
				"multiple complexes share the requested PNU"), false, requestId);
		});
	}

	@Override
	public OdcMetadataGapFillOutcome saveResolution(OdcMetadataGapFillTarget target,
		ComplexMetadataResolution resolution, UUID requestId) {
		return transaction.execute(status -> saveLocked(target, resolution, requestId));
	}

	private OdcMetadataGapFillOutcome saveLocked(OdcMetadataGapFillTarget target,
		ComplexMetadataResolution resolution, UUID requestId) {
		LockedComplex locked = lock(target.lookup().complexId());
		OdcMetadataGapFillOutcome duplicate = existingOutcome(locked.id(), requestId);
		if (duplicate != null) return duplicate;
		if (!Objects.equals(locked.pnu(), target.lookup().pnu()) || pnuCount(locked.pnu()) != 1) {
			return record(locked, ComplexMetadataResolution.ambiguous("ODC",
				"PNU changed or is no longer unique"), false, requestId);
		}
		ComplexMetadata candidate = resolution.metadata();
		if (candidate != null && conflicts(locked, candidate)) {
			return record(locked, ComplexMetadataResolution.ambiguous("ODC",
				"existing core metadata conflicts with ODC candidate"), false, requestId);
		}
		...
		boolean applied = !Objects.equals(locked.dongCnt(), dong)
			|| !Objects.equals(locked.unitCnt(), unit) || !Objects.equals(locked.useDate(), useDate);
		...
		ComplexMetadataResolution projected = new ComplexMetadataResolution(status,
			new ComplexMetadata(dong, unit, null, null, null, null, null, useDate), "ODC", null, null,
		...
		jdbc.sql("""
			...
			WHERE id=:id
			""").param("dong", dong).param("unit", unit).param("use_date", useDate)
			.param("status", status.name()).param("applied", applied).param("id", locked.id()).update();
		return record(locked, projected, applied, requestId);
	}

	private OdcMetadataGapFillOutcome record(LockedComplex locked, ComplexMetadataResolution resolution,
		boolean applied, UUID requestId) {
		Instant next = retryPolicy.nextAttemptAt(resolution.status(), resolution.failureKind(), locked.attempts() + 1,
		...
		int attemptNo = jdbc.sql("""
			...
			.param("kind", resolution.failureKind() == null ? null : resolution.failureKind().name())
			.param("reason", resolution.failureReason()).param("next_at", offset(next)).param("id", locked.id())
		...
		jdbc.sql("""
			...
			""").param("id", locked.id()).param("attempt", attemptNo).param("status", resolution.status().name())
			.param("kind", resolution.failureKind() == null ? null : resolution.failureKind().name())
			.param("reason", resolution.failureReason()).param("next_at", offset(next))
			.param("lookup_path", evidence.lookupPath().name()).param("requested_pnu", locked.pnu())
			.param("resolved_pnu", evidence.resolvedSourcePnu()).param("alias_id", evidence.aliasId())
			.param("candidate_count", evidence.candidateCount()).param("request_id", requestId)
	...
	private boolean conflicts(LockedComplex locked, ComplexMetadata candidate) {
		return differs(locked.dongCnt(), candidate.dongCnt()) || differs(locked.unitCnt(), candidate.unitCnt())
	...
	private OdcMetadataGapFillOutcome existingOutcome(long complexId, UUID requestId) {
		return jdbc.sql("""
			...
			""").param("id", complexId).param("request_id", requestId)
	...
	private LockedComplex lock(long id) {
		return jdbc.sql("""
			...
			""").param("id", id).query((rs, row) -> new LockedComplex(rs.getLong("id"), rs.getString("pnu"),
			rs.getInt("metadata_attempts"), (Integer) rs.getObject("dong_cnt"),
			(Integer) rs.getObject("unit_cnt"), rs.getObject("use_date", LocalDate.class))).single();
	}
	...
	private OdcMetadataGapFillTarget target(ResultSet rs, int row) throws SQLException {
		return new OdcMetadataGapFillTarget(new ComplexMetadataLookup(rs.getLong("id"), rs.getString("apt_seq"),
			rs.getString("name"), rs.getString("pnu"), rs.getString("address"), rs.getInt("metadata_attempts")),
	...
	private record LockedComplex(long id, String pnu, int attempts, Integer dongCnt, Integer unitCnt,
		LocalDate useDate) { }
}
