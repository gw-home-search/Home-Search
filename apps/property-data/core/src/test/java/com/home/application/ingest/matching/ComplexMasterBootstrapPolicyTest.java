...
class ComplexMasterBootstrapPolicyTest {
    ...
    void skipsExistingAptSeqWhenPnuIsUnavailable() {
        Optional<ComplexMasterBootstrapResult> result =
                policy.validateExistingAptSeqPnu("APT-501", Optional.empty(), Optional.of("1168010300101400001"));
    ...
    void skipsExistingAptSeqWhenComplexParcelIsUnavailable() {
        Optional<ComplexMasterBootstrapResult> result =
                policy.validateExistingAptSeqPnu("APT-501", Optional.of("1168010300101400001"), Optional.empty());
    ...
    void skipsExistingAptSeqWhenPnuConflicts() {
        Optional<ComplexMasterBootstrapResult> result = policy.validateExistingAptSeqPnu(
                "APT-501", Optional.of("1168010300107770001"), Optional.of("1168010300101400001"));

        assertThat(result).isPresent();
        assertThat(result.get().failureReason())
                .isEqualTo(
                        "master bootstrap skipped: aptSeq parcel pnu conflict aptSeq=APT-501 derivedPnu=1168010300107770001 complexPnu=1168010300101400001");
    }
    ...
    void validatesExistingComplexPkPnu() {
        ...
        assertThat(policy.validateExistingComplexPkPnu(
                        "RTMS:APT-501", "1168010300107770001", Optional.of("1168010300101400001")))
                .get()
                .extracting(ComplexMasterBootstrapResult::failureReason)
                .isEqualTo(
                        "master bootstrap skipped: complex_pk parcel pnu conflict complexPk=RTMS:APT-501 derivedPnu=1168010300107770001 complexPnu=1168010300101400001");
    }

    @Test
    @DisplayName("parcel을 만들거나 찾을 수 없으면 skip한다")
    void validatesParcelAvailability() {
        Optional<ComplexMasterBootstrapResult> result = policy.validateParcel("1168010300107770001", Optional.empty());
