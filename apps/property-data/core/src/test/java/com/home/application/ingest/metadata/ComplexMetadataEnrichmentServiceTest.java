...
class ComplexMetadataEnrichmentServiceTest {
    ...
    void enrichesPendingComplexesAndContinuesAfterRowFailure() {
        FakeRepository repository =
                new FakeRepository(List.of(lookup(501, 0), lookup(502, 0), lookup(503, 1), lookup(504, 0)));
        ...
        assertThat(repository.saved).extracting(SavedResolution::complexId).containsExactly(501L, 502L, 503L, 504L);
        assertThat(repository.saved)
                .extracting(saved -> saved.resolution().status())
                .containsExactly(
    ...
    private record SavedResolution(Long complexId, ComplexMetadataResolution resolution, Instant nextAttemptAt) {}
}
