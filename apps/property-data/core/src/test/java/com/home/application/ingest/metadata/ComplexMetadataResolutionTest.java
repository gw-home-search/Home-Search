...
class ComplexMetadataResolutionTest {
    ...
    void criticalMetadataRequiresDongUnitAndUseDate() {
        assertThat(metadata(8, 740, LocalDate.of(2015, 3, 20)).hasAllCriticalFields())
                .isTrue();
        assertThat(metadata(null, 740, LocalDate.of(2015, 3, 20)).hasAllCriticalFields())
                .isFalse();
        assertThat(metadata(8, null, LocalDate.of(2015, 3, 20)).hasAllCriticalFields())
                .isFalse();
        assertThat(metadata(8, 740, null).hasAllCriticalFields()).isFalse();
        assertThat(metadata(0, 740, LocalDate.of(2015, 3, 20)).hasAllCriticalFields())
                .isFalse();
        assertThat(metadata(8, 0, LocalDate.of(2015, 3, 20)).hasAllCriticalFields())
                .isFalse();
    }
    ...
    void classifyByCriticalMetadataCompleteness() {
        assertThat(ComplexMetadataResolution.classify("ODC", metadata(8, 740, LocalDate.of(2015, 3, 20)))
                        .status())
                .isEqualTo(ComplexMetadataStatus.RESOLVED);

        ComplexMetadataResolution partial = ComplexMetadataResolution.classify("BLD", metadata(null, 740, null));
