...
class ComplexMetadataRetryPolicyTest {
    ...
    void terminalStatusesDoNotScheduleRetry() {
        assertThat(policy.nextAttemptAt(ComplexMetadataStatus.RESOLVED, null, 1, NOW))
                .isEmpty();
    ...
    void partialAndSourceMissingUseLongBackoff() {
        ...
        assertThat(policy.nextAttemptAt(ComplexMetadataStatus.PARTIAL, null, 4, NOW))
                .isEmpty();

        assertThat(policy.nextAttemptAt(
                        ComplexMetadataStatus.UNAVAILABLE, ComplexMetadataFailureKind.SOURCE_MISSING, 1, NOW))
                .contains(NOW.plusSeconds(2_592_000));
        assertThat(policy.nextAttemptAt(
                        ComplexMetadataStatus.UNAVAILABLE, ComplexMetadataFailureKind.SOURCE_MISSING, 2, NOW))
                .contains(NOW.plusSeconds(7_776_000));
        assertThat(policy.nextAttemptAt(
                        ComplexMetadataStatus.UNAVAILABLE, ComplexMetadataFailureKind.SOURCE_MISSING, 3, NOW))
                .contains(NOW.plusSeconds(15_552_000));
        assertThat(policy.nextAttemptAt(
                        ComplexMetadataStatus.UNAVAILABLE, ComplexMetadataFailureKind.SOURCE_MISSING, 8, NOW))
                .contains(NOW.plusSeconds(15_552_000));
    }
    ...
    void inputInsufficientAndPermanentFailuresDoNotRetry() {
        assertThat(policy.nextAttemptAt(
                        ComplexMetadataStatus.UNAVAILABLE, ComplexMetadataFailureKind.INPUT_INSUFFICIENT, 1, NOW))
                .isEmpty();
