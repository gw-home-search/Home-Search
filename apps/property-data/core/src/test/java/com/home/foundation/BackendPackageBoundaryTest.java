...
class BackendPackageBoundaryTest {
	...
	private static final ArchRule DOMAIN_BOUNDARY = noClasses()
		.that().resideInAPackage("com.home.domain..")
	...
	private static final ArchRule APPLICATION_BOUNDARY = noClasses()
		.that().resideInAPackage("com.home.application..")
		.should().dependOnClassesThat().resideInAnyPackage(
			"com.home.infrastructure..",
	...
	private static final ArchRule JDBC_PERSISTENCE_BOUNDARY = noClasses()
		.that().resideInAPackage("com.home.infrastructure.persistence..")
		.should().dependOnClassesThat().resideInAnyPackage(
			"jakarta.persistence..",
			"javax.persistence..",
			"org.hibernate.."
	...
	private static final ArchRule EXTERNAL_ADAPTER_BOUNDARY = noClasses()
		.that().resideInAPackage("com.home.infrastructure.external..")
		.should().dependOnClassesThat().resideInAPackage("com.home.infrastructure.persistence..");
	...
	void roleRulesRejectForbiddenDependencyFixtures() {
		...
		assertThatThrownBy(() -> DOMAIN_BOUNDARY.check(domainFixture))
			.isInstanceOf(AssertionError.class);
		assertThatThrownBy(() -> APPLICATION_BOUNDARY.check(applicationFixture))
			.isInstanceOf(AssertionError.class);
	}
	...
	void persistenceAllowsJdbcButRejectsJpaFixture() {
		...
		assertThatThrownBy(() -> JDBC_PERSISTENCE_BOUNDARY.check(jpaFixture))
			.isInstanceOf(AssertionError.class);
	}
}
