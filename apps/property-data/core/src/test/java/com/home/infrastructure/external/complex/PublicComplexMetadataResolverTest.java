...
class PublicComplexMetadataResolverTest {
    ...
    void resolvesCombinedMetadataFromOdcloudAndBuildingApis() {
        ...
        MockRestServiceServer odcloudServer =
                MockRestServiceServer.bindTo(odcloudBuilder).build();
        RestClient.Builder bldBuilder = RestClient.builder().baseUrl("https://apis.example.test");
        MockRestServiceServer bldServer =
                MockRestServiceServer.bindTo(bldBuilder).build();
        ...
        odcloudServer
                .expect(requestTo(startsWith("https://odcloud.example.test" + ODC_APT_PATH)))
                .andExpect(request ->
                        assertThat(request.getURI().getRawQuery()).contains("cond%5BPNU::EQ%5D=1168010300107770001"))
        ...
        bldServer
                .expect(requestTo(
                        startsWith("https://apis.example.test/1613000/BldRgstHubService/getBrRecapTitleInfo")))
    ...
    void resolvesApprovedLegacyPnuAliasAfterCanonicalMiss() {
        ...
        PublicComplexMetadataResolver resolver = new PublicComplexMetadataResolver(
                ...
                "/title",
                false,
                pnu -> java.util.Optional.of(
                        new com.home.application.ingest.metadata.OdcloudPnuPrefixAlias(7L, "41461262", "41461360")));
        odcloudServer
                .expect(requestTo(startsWith("https://odcloud.example.test" + ODC_APT_PATH)))
                .andExpect(request ->
                        assertThat(request.getURI().getRawQuery()).contains("cond%5BPNU::EQ%5D=4146126200109010000"))
                .andRespond(withSuccess("{\"data\": []}", MediaType.APPLICATION_JSON));
        odcloudServer
                .expect(requestTo(startsWith("https://odcloud.example.test" + ODC_APT_PATH)))
                .andExpect(request ->
                        assertThat(request.getURI().getRawQuery()).contains("cond%5BPNU::EQ%5D=4146136000109010000"))
                .andRespond(withSuccess("""
				{"data":[{"PNU":"4146136000109010000","COMPLEX_PK":"APT-501","DONG_CNT":8,"UNIT_CNT":740,"USEAPR_DT":"20150320"}]}
				""", MediaType.APPLICATION_JSON));

        var resolution = resolver.resolve(
                new ComplexMetadataLookup(501L, "APT-501", "Legacy Apartment", "4146126200109010000", "양지읍"));
    ...
    void ambiguousOdcloudPnuCandidatesReturnAmbiguousResolution() {
        ...
        var resolution = resolver.resolve(new ComplexMetadataLookup(
                501L, "APT-LIVE-501", "Live Sample Apartment", "1168010300107770001", "Sample address"));
    ...
    void picksNameMatchedCandidateAmongMultipleOdcloudPnuCandidates() {
        ...
        var resolution = resolver.resolve(
                new ComplexMetadataLookup(501L, "APT-501", "한국아파트", "4115010400107270001", "신곡동 727-1"));
    ...
    void doesNotAutoResolvePartialNameMatch() {
        ...
        var resolution =
                resolver.resolve(new ComplexMetadataLookup(501L, "APT-501", "한국", "4115010400107270001", "신곡동"));
    ...
    void keepsAmbiguousWhenNameScoresTieAmongCandidates() {
        ...
        var resolution = resolver.resolve(
                new ComplexMetadataLookup(501L, "APT-501", "삼성래미안", "1168010300107770001", "Sample address"));
    ...
    void resolverDoesNotDoubleEncodePortalEncodedServiceKeys() {
        ...
        assertThat(resolver.resolve(new ComplexMetadataLookup(
                                501L, "APT-LIVE-501", "Live Sample Apartment", "1168010300107770001", "Sample address"))
                        .status())
                .isEqualTo(ComplexMetadataStatus.RESOLVED);
        odcloudServer.verify();
        bldServer.verify();
    }
    ...
    void ambiguousBuildingApartmentCandidatesReturnAmbiguousResolution() {
        ...
        var resolution = resolver.resolve(
                new ComplexMetadataLookup(501L, "APT-LIVE-501", "Live Sample Apartment", "1168010300107770001", null));
    ...
    void resolvesOdcloudMetadataByCanonicalPnuWhenAddressIsMissing() {
        ...
        bldServer
				...
				""", MediaType.APPLICATION_JSON));
        bldServer
                .expect(requestTo(startsWith("https://apis.example.test/1613000/BldRgstHubService/getBrTitleInfo")))
                .andExpect(queryParam("serviceKey", "BLD-KEY"))
                .andRespond(withSuccess("""
				{
				  "response": {
