...
class OdcloudIdentityExternalApiConfigurationTest {
    ...
    private final ApplicationContextRunner contextRunner =
            new ApplicationContextRunner().withUserConfiguration(OdcloudIdentityExternalApiConfiguration.class);
    ...
    void odcloudIdentityFallbackCreatesResolverWhenEnabled() {
        contextRunner
                .withPropertyValues("complex.identity.odcloud.enabled=true", "odcloud.data.od-service-key=ODC-KEY")
    ...
    void odcloudCoordinateIdentityGateCreatesVerifierWhenEnabled() {
        contextRunner
                .withPropertyValues(
                        "complex.coordinate.identity.odcloud.enabled=true", "odcloud.data.od-service-key=ODC-KEY")
                .run(context -> {
    ...
    void odcloudCoordinateIdentityVerifierIsDefaultWhenServiceKeyConfigured() {
        contextRunner.withPropertyValues("odcloud.data.od-service-key=ODC-KEY").run(context -> {
            assertThat(context).hasSingleBean(ComplexCoordinateIdentityVerifier.class);
            assertThat(context.getBean(ComplexCoordinateIdentityVerifier.class))
                    .isInstanceOf(OdcloudComplexCoordinateIdentityVerifier.class);
        });
    ...
    void odcloudCoordinateIdentityVerifierCanBeExplicitlyDisabled() {
        contextRunner
                .withPropertyValues(
                        "complex.coordinate.identity.odcloud.enabled=false", "odcloud.data.od-service-key=ODC-KEY")
