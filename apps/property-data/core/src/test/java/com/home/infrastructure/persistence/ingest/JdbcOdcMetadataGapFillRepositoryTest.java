...
class JdbcOdcMetadataGapFillRepositoryTest extends JdbcMigrationTestSupport {
    ...
    void cancelsWholeProjectionWhenAnyCoreValueConflicts() {
        ...
        ComplexMetadataResolution resolution = ComplexMetadataResolution.resolved(
                        "ODC", new ComplexMetadata(9, 740, null, null, null, null, null, LocalDate.of(2015, 1, 1)))
                .withLookupEvidence(new ComplexMetadataLookupEvidence(
                        ComplexMetadataLookupPath.CANONICAL_PNU,
                        target.lookup().pnu(),
                        target.lookup().pnu(),
                        null,
                        1));
        ...
        assertThat(jdbcClient
                        .sql("SELECT unit_cnt FROM complex WHERE id=501")
                        .query(Integer.class)
                        .optional())
                .isEmpty();
        assertThat(jdbcClient
    ...
    void fillsOnlyNullCoreValuesAndMakesSameRequestIdempotent() {
        ...
        var target = repository.findTargets(20, null, 1000, REQUEST_ID).get(0);
        ComplexMetadataResolution resolution = ComplexMetadataResolution.resolved(
                        "ODC", new ComplexMetadata(8, 740, null, null, null, null, null, LocalDate.of(2015, 1, 1)))
                .withLookupEvidence(new ComplexMetadataLookupEvidence(
                        ComplexMetadataLookupPath.CANONICAL_PNU,
                        target.lookup().pnu(),
                        target.lookup().pnu(),
        ...
        assertThat(jdbcClient
                        .sql("SELECT dong_cnt||':'||unit_cnt||':'||use_date FROM complex WHERE id=501")
                        .query(String.class)
                        .single())
                .isEqualTo("8:740:2015-01-01");
        assertThat(jdbcClient
                        .sql("SELECT count(*) FROM complex_metadata_enrichment_attempt WHERE complex_id=501")
                        .query(Integer.class)
                        .single())
                .isOne();
    }
    ...
    void storesSourceMissingWithoutProjection() {
        ...
        ComplexMetadataResolution unavailable = ComplexMetadataResolution.unavailable("ODC", "source unavailable")
                .withLookupEvidence(new ComplexMetadataLookupEvidence(
                        ComplexMetadataLookupPath.CANONICAL_PNU, target.lookup().pnu(), null, null, 0));
        ...
        assertThat(jdbcClient
                        .sql(
                                "SELECT next_attempt_at IS NOT NULL FROM complex_metadata_enrichment_attempt WHERE complex_id=501")
                        .query(Boolean.class)
                        .single())
                .isTrue();
    }
    ...
    void newGapFillRequestSkipsPreviouslyAttemptedComplexes() {
        ...
        ComplexMetadataResolution unavailable = ComplexMetadataResolution.unavailable("ODC", "source unavailable")
                .withLookupEvidence(new ComplexMetadataLookupEvidence(
                        ComplexMetadataLookupPath.CANONICAL_PNU, first.lookup().pnu(), null, null, 0));
    ...
    private void seed(long id, long parcelId, String pnu, Integer dongCnt, Integer unitCnt, LocalDate useDate) {
        jdbcClient
                .sql("INSERT INTO parcel(id,pnu,address) VALUES (:id,:pnu,'Sample')")
                .param("id", parcelId)
                .param("pnu", pnu)
                .update();
        jdbcClient
			...
			""")
                .param("id", id)
                .param("parcel_id", parcelId)
                .param("complex_pk", "RTMS:" + id)
                .param("apt_seq", "APT-" + id)
                .param("dong", dongCnt)
                .param("unit", unitCnt)
