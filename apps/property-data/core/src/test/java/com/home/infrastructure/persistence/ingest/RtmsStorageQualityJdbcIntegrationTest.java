...
class RtmsStorageQualityJdbcIntegrationTest extends JdbcPostgresTestSupport {
    ...
    void samePnuDifferentAptSeqStoresSeparateComplexTradesAndGroupedParcelMarker() {
        OpenApiTradeIngestService service = ingestService(coordinates(Map.of(
                "1168010300101850000",
                new ParcelCoordinate(new BigDecimal("37.4890754"), new BigDecimal("127.0715119")))));
        ...
        assertThat(rawStatusCounts()).containsEntry("NORMALIZED", 2L).hasSize(1);
        ...
        assertThat(complexesForPnu("1168010300101850000"))
                .containsExactlyInAnyOrder(tuple("11680-290", "개포주공6단지", 1L), tuple("11680-578", "개포주공7단지", 1L));
        ...
        assertThat(new JdbcTradeHistoryReader(jdbcClient).findTradeList(parcelId("1168010300101850000"), null, 0, 25))
    ...
    void cancellationStaysAsRawEvidenceAndDeletesPublicTrade() {
        ...
        IngestResult inserted = service.ingest(
                batch(List.of(rtmsItem("APT-501", "Sample Apartment", 1, "125,000", 12, "84.93", "101", "140-1"))));
        IngestResult canceled = service.ingest(batch(
                List.of(canceledRtmsItem("APT-501", "Sample Apartment", 1, "125,000", 12, "84.93", "101", "140-1"))));
    ...
    private ComplexMarkerQuery bounds() {
        return new ComplexMarkerQuery(37.45, 127.00, 37.55, 127.12, null, null, null, null, null, null, null, null);
    }
    ...
    private Map<String, Long> rawStatusCounts() {
        return jdbcClient
			...
			ORDER BY status
			""")
                .query((resultSet, rowNumber) -> Map.entry(resultSet.getString("status"), resultSet.getLong("count")))
                .list()
                .stream()
                .collect(java.util.stream.Collectors.toMap(
                        Map.Entry::getKey, Map.Entry::getValue, Long::sum, java.util.LinkedHashMap::new));
    }
    ...
    private Long parcelId(String pnu) {
        return jdbcClient
                .sql("SELECT id FROM parcel WHERE pnu = :pnu")
                .param("pnu", pnu)
                .query(Long.class)
                .single();
    }
        ...
        return jdbcClient
			...
			""")
                ...
                .query((resultSet, rowNumber) -> tuple(
                        resultSet.getString("apt_seq"), resultSet.getString("name"), resultSet.getLong("trade_count")))
                .list();
    }
