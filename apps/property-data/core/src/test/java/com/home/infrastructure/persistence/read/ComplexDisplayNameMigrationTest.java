...
class ComplexDisplayNameMigrationTest extends JdbcMigrationTestSupport {
	...
	void v8BackfillsProjectionWithoutChangingSourceIdentityAndKeepsItSynchronized() {
		...
		assertThat(jdbcClient.sql("""
			...
			.isEqualTo("응봉동 대림(2차)|응봉동대림2차");
		assertThat(identityChecksum()).isEqualTo(identityBefore);

		jdbcClient.sql("UPDATE region SET name = '금호동' WHERE code = '11200108'").update();
		assertThat(jdbcClient.sql("SELECT display_name FROM complex WHERE id = 4677")
			.query(String.class).single()).isEqualTo("금호동 대림(2차)");

		jdbcClient.sql("UPDATE complex SET trade_name = '금호동대림' WHERE id = 4677").update();
		assertThat(jdbcClient.sql("SELECT display_name FROM complex WHERE id = 4677")
			.query(String.class).single()).isEqualTo("금호동 대림");
	...
	private String indexDefinition(String indexName) {
		return jdbcClient.sql("""
			...
			""").param("indexName", indexName).query(String.class).optional().orElse("");
	}
}
