...
class JdbcComplexSearchReaderQueryShapeTest {
    ...
    @Test
    @DisplayName("긴 단일 검색어의 prefix 단계는 btree index가 가능한 direct parameter를 사용한다")
    void longSingleTermPrefixUsesDirectBoundPatterns() {
        PropertySearchTerms terms = PropertySearchTerms.from("마포래미안푸르지오");

        String candidates = JdbcComplexSearchReader.prefixCandidatesFor(terms);

        assertThat(candidates)
                .contains("LIKE :rawPrefix")
                .contains("LIKE :normalizedPrefix")
                .doesNotContain("query_tokens");
        assertThat(terms.rawPrefixPattern()).isEqualTo("마포래미안푸르지오%");
        assertThat(terms.normalizedPrefixPattern()).isEqualTo("마포래미안푸르지오%");
    }
