...
verify_info(){
  ...
  [[ "$(catalog | paste -sd, -)" == '1,2,4,5,6,7,8,9,10,11' && "$(catalog | tail -1)" == "${target}" ]] || policy_error 'resolved SQL catalog 또는 target이 예상과 다릅니다.'
  ...
  [[ "${versions}" == '1,2,4,5,6,7,8,9,10,11' ]] || policy_error 'resolved version set이 예상과 다릅니다.'
