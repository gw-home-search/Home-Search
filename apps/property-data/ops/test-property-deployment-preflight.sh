...
info_with_v2(){
  ...
  printf '{"migrations":[{"version":"1","type":"SQL","state":"Success"},{"version":"2","type":"%s","state":"%s"},{"version":"4","type":"SQL","state":"Success"},{"version":"5","type":"SQL","state":"Success"},{"version":"6","type":"SQL","state":"Success"},{"version":"7","type":"SQL","state":"Success"},{"version":"8","type":"SQL","state":"Success"}%s]}' "${type}" "${state}" "${extra}"
}
...
: > "${TEMP}/docker.log"
reject 2 invoke env PROPERTY_MIGRATOR_JDBC_URL='jdbc:postgresql://db:5432/wrong_database' "${SCRIPT}" before 8
[[ ! -s "${TEMP}/docker.log" ]]
...
reject 2 invoke env FAKE_INFO="${pending}" "${SCRIPT}" before 7
...
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${jdbc}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${deleted}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${out_of_order}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${missing}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${ignored}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${failed}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${duplicate}" FAKE_ROWS="${rows}" "${SCRIPT}" after 8
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${success}" FAKE_ROWS="${failed_rows}" "${SCRIPT}" after 8
