...
bash -n "${SCRIPT}"
reject 2 env -u PROPERTY_MIGRATOR_DB_PASSWORD "${SCRIPT}" before 41
reject 2 invoke "${SCRIPT}" before
reject 2 invoke "${SCRIPT}" before x
: > "${TEMP}/docker.log"
reject 2 invoke "${SCRIPT}" legacy-before 41
[[ ! -s "${TEMP}/docker.log" ]]
: > "${TEMP}/docker.log"
reject 2 invoke env PROPERTY_MIGRATOR_JDBC_URL='jdbc:postgresql://db:5432/wrong_database' "${SCRIPT}" before 41
[[ ! -s "${TEMP}/docker.log" ]]
reject 2 invoke env PROPERTY_MIGRATOR_JDBC_URL='jdbc:postgresql://migrator:authority-sentinel@db:5432/home_search' "${SCRIPT}" before 41
[[ ! -s "${TEMP}/docker.log" ]]
reject 2 invoke env PROPERTY_MIGRATOR_JDBC_URL='jdbc:postgresql://db:5432/home_search?Password=query-sentinel' "${SCRIPT}" before 41
[[ ! -s "${TEMP}/docker.log" ]]
! grep -Fq authority-sentinel "${TEMP}/out" "${TEMP}/err" "${TEMP}/docker.log"
! grep -Fq query-sentinel "${TEMP}/out" "${TEMP}/err" "${TEMP}/docker.log"
reject 2 invoke env FAKE_DATABASE=wrong FAKE_INFO="${pending}" "${SCRIPT}" before 41
reject 2 invoke env FAKE_INFO="${pending}" "${SCRIPT}" before 8
reject 2 invoke env FAKE_RELATIONS=1 FAKE_INFO="${pending}" "${SCRIPT}" before 41
[[ "$(invoke env FAKE_INFO="${pending}" "${SCRIPT}" before 41)" == 'service=property-data phase=before target=41 state=EMPTY' ]]
[[ "$(invoke env FAKE_HISTORY=true FAKE_RELATIONS=32 FAKE_INFO="${success}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41)" == 'service=property-data phase=after target=41 state=READY' ]]
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${jdbc}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${deleted}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${out_of_order}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${missing}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${ignored}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${failed}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${duplicate}" FAKE_ROWS="${rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${success}" FAKE_ROWS="${failed_rows}" "${SCRIPT}" after 41
reject 2 invoke env FAKE_HISTORY=true FAKE_INFO="${success}" FAKE_ROWS="${rows}"$'<null>|BASELINE|t\n' "${SCRIPT}" after 41
reject 1 invoke env FAKE_HISTORY=true FAKE_INFO="${success}" FAKE_ROWS="${rows}" FAKE_VALIDATE='{"error":{"message":"validation failed"}}' FAKE_VALIDATE_EXIT=1 "${SCRIPT}" after 41
[[ "$(invoke env FAKE_HISTORY=true FAKE_RELATIONS=32 FAKE_INFO="${success}" FAKE_ROWS="${rows}" FAKE_VALIDATE= "${SCRIPT}" after 41)" == 'service=property-data phase=after target=41 state=READY' ]]
