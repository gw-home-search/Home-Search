...
```bash
docker compose -f infra/docker-compose.local.yml up -d postgis api
...
```bash
k6 run -e SCENARIO=smoke -e BASE_URL=http://localhost:8080 apps/property-data/perf/k6/map-marker-baseline.js
```
...
k6 run \
  ...
  -e TARGET_RPS=1 \
  -e SUMMARY_EXPORT=apps/property-data/perf/results/map-marker-baseline.json \
...
```bash
k6 run -e SCENARIO=smoke -e COMPLEX_WEIGHT=1 -e REGION_WEIGHT=0 apps/property-data/perf/k6/map-marker-baseline.js
k6 run -e SCENARIO=smoke -e COMPLEX_WEIGHT=0 -e REGION_WEIGHT=1 apps/property-data/perf/k6/map-marker-baseline.js
```
