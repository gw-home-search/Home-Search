...
final class LegacyCoordinateSourceFingerprint {
    private static final Set<String> EXPECTED_TABLES = Set.of("coordinate_snapshot_run", "parcel_coordinate_snapshot");
    ...
    private static final Set<String> EXPECTED_CONSTRAINTS = Set.of(
            "coordinate_snapshot_run_duplicate_pnu_count_check",
            "coordinate_snapshot_run_file_count_check",
            "coordinate_snapshot_run_invalid_count_check",
            "coordinate_snapshot_run_pkey",
            "coordinate_snapshot_run_pnu_count_check",
            "coordinate_snapshot_run_raw_feature_count_check",
            "coordinate_snapshot_run_region_count_check",
            "coordinate_snapshot_run_status_check",
            "coordinate_snapshot_run_synced_parcel_count_check",
    ...
    LegacyFingerprintEvidence verify(DataSource dataSource) throws SQLException {
        try (Connection connection = dataSource.getConnection()) {
            ...
            Set<String> tables = strings(
                    connection,
                    "SELECT table_name FROM information_schema.tables WHERE table_schema='reference' AND table_type='BASE TABLE' ORDER BY table_name");
            ...
            Set<String> constraints = strings(connection, """
                ...
                """);
            Set<String> indexes = strings(
                    connection, "SELECT indexname FROM pg_indexes WHERE schemaname='reference' ORDER BY indexname");
    ...
    record LegacyFingerprintEvidence(Map<String, Long> estimatedRows) {
        LegacyFingerprintEvidence {
            estimatedRows = Map.copyOf(estimatedRows);
        }
    }

    static final class LegacyFingerprintMismatchException extends RuntimeException {
        LegacyFingerprintMismatchException(String message) {
            super(message);
        }
    }
}
