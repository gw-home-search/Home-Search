...
public class FavoriteController {
    ...
    public FavoriteController(SaveFavoriteComplex save, RemoveFavoriteComplex remove,
            GetFavoriteComplex get, ListFavoriteComplexes list) {
        this.save = save; this.remove = remove; this.get = get; this.list = list;
    }

    @GetMapping("/api/v1/favorites")
    FavoriteListResponse list(@AuthenticationPrincipal AuthenticatedUserPrincipal principal,
            @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int size) {
        var result = list.execute(principal.userId(), page, size);
        List<FavoriteItemResponse> content = result.content().stream()
            .map(value -> new FavoriteItemResponse(value.complexId(), value.savedAt())).toList();
    ...
    FavoriteStatusResponse get(@AuthenticationPrincipal AuthenticatedUserPrincipal principal,
    ...
    record FavoriteListResponse(List<FavoriteItemResponse> content, int page, int size, long totalElements, int totalPages) { }
}
