...
class UserPersistenceConcurrencyTest {
    private static final PostgreSQLContainer<?> POSTGRES = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("home_search_user");
    ...
    static {
        try {
            ...
            Flyway.configure().dataSource(POSTGRES.getJdbcUrl(), POSTGRES.getUsername(), POSTGRES.getPassword())
                    ...
                    .schemas("users").defaultSchema("users").load().migrate();
            ...
            PRIVATE_KEY.toFile().deleteOnExit(); PUBLIC_KEY.toFile().deleteOnExit();
            OAUTH_SERVER = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
            OAUTH_SERVER.createContext("/token", exchange -> respond(exchange,
                    "{\"access_token\":\"provider-access\",\"token_type\":\"Bearer\",\"expires_in\":300}"));
            OAUTH_SERVER.createContext("/userinfo", exchange -> respond(exchange,
                    "{\"id\":98765,\"kakao_account\":{\"profile\":{\"nickname\":\"OAuth 사용자\"}}}"));
            OAUTH_SERVER.start();
    ...
    static void properties(DynamicPropertyRegistry registry) {
        ...
        registry.add("GOOGLE_OAUTH_CLIENT_ID", () -> "google-test"); registry.add("GOOGLE_OAUTH_CLIENT_SECRET", () -> "google-secret");
        registry.add("KAKAO_OAUTH_CLIENT_ID", () -> "kakao-test"); registry.add("KAKAO_OAUTH_CLIENT_SECRET", () -> "kakao-secret");
        registry.add("NAVER_OAUTH_CLIENT_ID", () -> "naver-test"); registry.add("NAVER_OAUTH_CLIENT_SECRET", () -> "naver-secret");
        ...
        registry.add("home.jwt.active-kid", () -> "test-key"); registry.add("home.jwt.private-key-path", PRIVATE_KEY::toString);
    ...
    @Autowired OAuthLoginFacade login;
    @Autowired RefreshTokenService refresh;
    @Autowired AccessTokenIssuer accessTokens;
    @Autowired JdbcClient jdbc;
    @Autowired MockMvc mockMvc;
    @Autowired SaveFavoriteComplex saveFavorite;

    @Test
    void serializesFirstLoginAndAllowsExactlyOneRefreshRotation() throws Exception {
        try (var executor = Executors.newFixedThreadPool(2)) {
            Callable<Long> loginTask = () -> login.login(OAuthProvider.KAKAO,
                    new OAuthProfile("concurrent-subject", new UserProfile("동시 사용자", null, null))).userId();
            var firstLogin = executor.submit(loginTask); var secondLogin = executor.submit(loginTask);
            long firstId = firstLogin.get(); long secondId = secondLogin.get();
            assertThat(secondId).isEqualTo(firstId);
            assertThat(jdbc.sql("SELECT count(*) FROM users.oauth_identity WHERE provider='KAKAO' AND provider_subject='concurrent-subject'")
                    .query(Long.class).single()).isEqualTo(1L);

            String raw = refresh.issue(firstId).rawToken();
            Callable<Boolean> rotateTask = () -> { try { refresh.rotate(raw); return true; } catch (InvalidRefreshTokenException ignored) { return false; } };
            var firstRotate = executor.submit(rotateTask); var secondRotate = executor.submit(rotateTask);
    ...
    void enforcesUserJwtAndDenyAllThroughSecurityChain() throws Exception {
        var user = login.login(OAuthProvider.GOOGLE,
        ...
        for (String rejected : java.util.List.of(
                signedAccessToken("user-service", "wrong-audience", user.userId(), "USER", Instant.now(), Duration.ofMinutes(15)),
                signedAccessToken("admin-service", "home-search-user-api", user.userId(), "ADMIN", Instant.now(), Duration.ofMinutes(15)),
                signedAccessToken("user-service", "home-search-user-api", user.userId(), "USER", Instant.now().minus(Duration.ofMinutes(20)), Duration.ofMinutes(15)))) {
    ...
    void rotatesRefreshOnlyForAllowedOriginThroughSecurityChain() throws Exception {
        var user = login.login(OAuthProvider.NAVER,
                new OAuthProfile("refresh-chain-user", new UserProfile("갱신 사용자", null, null)));
    ...
    void favoriteApiUsesJwtUserOnlyAndKeepsCorsOnAuthenticationFailure() throws Exception {
        var first = login.login(OAuthProvider.GOOGLE,
                new OAuthProfile("favorite-user-a", new UserProfile("관심 사용자 A", null, null)));
        var second = login.login(OAuthProvider.GOOGLE,
                new OAuthProfile("favorite-user-b", new UserProfile("관심 사용자 B", null, null)));
        ...
        mockMvc.perform(get("/api/v1/favorites").header(HttpHeaders.AUTHORIZATION, "Bearer " + firstToken))
                .andExpect(status().isOk()).andExpect(jsonPath("$.content[0].complexId").value(501))
        ...
        mockMvc.perform(get("/api/v1/favorites/0").header(HttpHeaders.AUTHORIZATION, "Bearer " + firstToken))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("INVALID_COMPLEX_ID"));
        mockMvc.perform(get("/api/v1/favorites").param("size", "101").header(HttpHeaders.AUTHORIZATION, "Bearer " + firstToken))
    ...
    void serializesConcurrentFavoriteAtTheTwoHundredItemLimit() throws Exception {
        var user = login.login(OAuthProvider.NAVER,
                new OAuthProfile("favorite-limit-user", new UserProfile("한도 사용자", null, null)));
        jdbc.sql("INSERT INTO users.favorite_complex(user_id,complex_id,saved_at) SELECT :userId, value, now() FROM generate_series(1,199) value")
        ...
        assertThat(jdbc.sql("SELECT count(*) FROM users.favorite_complex WHERE user_id=:userId")
                .param("userId", user.userId()).query(Long.class).single()).isEqualTo(200L);
    }

    @Test
    void validatesOAuthStateAndDeletesSessionAfterCallback() throws Exception {
        OAuthFlow mismatch = authorizeKakao();
        mockMvc.perform(get("/login/oauth2/code/kakao").param("code", "stub-code").param("state", "wrong-state")
                        .cookie(mismatch.cookie()))
                .andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("http://localhost:5173/auth/failure"));

        OAuthFlow expired = authorizeKakao();
        jdbc.sql("DELETE FROM users.oauth_session").update();
        mockMvc.perform(get("/login/oauth2/code/kakao").param("code", "stub-code").param("state", expired.state())
                        .cookie(expired.cookie()))
                .andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("http://localhost:5173/auth/failure"));

        OAuthFlow success = authorizeKakao();
        mockMvc.perform(get("/login/oauth2/code/kakao").param("code", "stub-code").param("state", success.state())
                        .cookie(success.cookie()))
                .andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("http://localhost:5173/auth/success"))
                .andExpect(header().string("Set-Cookie", org.hamcrest.Matchers.containsString("refresh_token=")));
        assertThat(jdbc.sql("SELECT count(*) FROM users.oauth_session_attributes a JOIN users.oauth_session s ON s.primary_id=a.session_primary_id WHERE s.session_id=:sessionId")
                .param("sessionId", decodedSessionId(success.cookie())).query(Long.class).single()).isZero();
        assertThat(jdbc.sql("SELECT count(*) FROM users.oauth_identity WHERE provider='KAKAO' AND provider_subject='98765'")
                .query(Long.class).single()).isEqualTo(1L);
    }

    private OAuthFlow authorizeKakao() throws Exception {
        var response = mockMvc.perform(get("/oauth2/authorization/kakao")).andExpect(status().is3xxRedirection()).andReturn().getResponse();
        var cookie = response.getCookie("OAUTH_SESSION");
        assertThat(cookie).isNotNull();
        String state = java.util.Arrays.stream(URI.create(response.getRedirectedUrl()).getRawQuery().split("&"))
                .map(value -> value.split("=", 2)).filter(parts -> parts[0].equals("state"))
                .map(parts -> URLDecoder.decode(parts[1], StandardCharsets.UTF_8)).findFirst().orElseThrow();
        return new OAuthFlow(cookie, state);
    }
    private boolean saveFavoriteResult(long userId, long complexId) {
        try { saveFavorite.execute(userId, complexId); return true; }
        catch (FavoriteLimitReachedException ignored) { return false; }
    }

    private static String oauthBaseUrl() { return "http://127.0.0.1:" + OAUTH_SERVER.getAddress().getPort(); }
    ...
    private static String signedAccessToken(String issuer, String audience, long userId, String role,
                                            Instant issuedAt, Duration lifetime) throws Exception {
        return new Rs256JwtCodec(Clock.fixed(issuedAt, ZoneOffset.UTC)).issue(new JwtIssueRequest(
                issuer, audience, Long.toString(userId), UUID.randomUUID().toString(), "test-key", lifetime,
                Map.of("role", role)), RsaPemKeys.privateKey(Files.readString(PRIVATE_KEY)));
    }
    @AfterAll static void stopResources() { OAUTH_SERVER.stop(0); POSTGRES.stop(); }
