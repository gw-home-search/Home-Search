...
class UserApiContractTest {
    ...
    void rotatesRefreshCookieBeforeReturningAccessToken() throws Exception {
        var refresh=mock(RefreshTokenService.class);AccessTokenIssuer access=mock(AccessTokenIssuer.class);
        when(refresh.rotate("old")).thenReturn(new RotatedRefreshToken(42,"new",Instant.now().plus(Duration.ofDays(30))));
        when(access.issue(42)).thenReturn("signed-access");
        var mvc=MockMvcBuilders.standaloneSetup(new AuthController(refresh,access,new RefreshTokenCookieFactory(true,Duration.ofDays(30),"prod"))).build();
        mvc.perform(post("/auth/access").cookie(new jakarta.servlet.http.Cookie("refresh_token","old")))
                .andExpect(status().isOk()).andExpect(jsonPath("$.accessToken").value("signed-access"))
                .andExpect(header().string(HttpHeaders.SET_COOKIE,org.hamcrest.Matchers.allOf(
                        ...
                        org.hamcrest.Matchers.containsString("SameSite=Lax"),org.hamcrest.Matchers.containsString("Path=/auth"))));
    }

    @Test
    void logoutWithoutCookieStillClearsCookie() throws Exception {
        var mvc=MockMvcBuilders.standaloneSetup(new AuthController(mock(RefreshTokenService.class),mock(AccessTokenIssuer.class),new RefreshTokenCookieFactory(true,Duration.ofDays(30),"prod"))).build();
        mvc.perform(post("/auth/logout")).andExpect(status().isNoContent())
                .andExpect(header().string(HttpHeaders.SET_COOKIE,org.hamcrest.Matchers.allOf(
                        org.hamcrest.Matchers.containsString("Max-Age=0"),org.hamcrest.Matchers.containsString("Path=/auth"))));
    }

    @Test
    void returnsExactMeFieldsWithoutEmail() throws Exception {
        var users=mock(CurrentUserQueryService.class);when(users.find(42)).thenReturn(new OAuthLoginResult(42,OAuthProvider.GOOGLE,new UserProfile("홍길동","hidden@example.com",null)));
