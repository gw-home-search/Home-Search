...
public class JpaRefreshTokenRepository implements RefreshTokenRepository {
    ...
    public void replaceActive(ActiveRefreshToken token) {
        repository.replaceActive(token.userId(), token.tokenHash(), token.issuedAt(), token.expiresAt());
    }
    ...
    public Optional<ActiveRefreshToken> findActiveByHash(String hash) {
        return repository
                ...
                .map(entity -> new ActiveRefreshToken(
                        entity.userId(), entity.tokenHash(), entity.issuedAt(), entity.expiresAt()));
    }

    @Override
    @Transactional
    public boolean rotateActive(String expectedHash, ActiveRefreshToken replacement, Instant now) {
        return repository.rotate(expectedHash, replacement.tokenHash(), now, replacement.expiresAt()) == 1;
    }

    @Override
    @Transactional
    public void revokeByHash(String hash, Instant now) {
        repository.revoke(hash, now);
    }
}
