...
describe('coordinate override admin API client 계약', () => {
  it('coordinate-pending 목록을 documented admin endpoint에서 조회한다', async () => {
    ...
    await expect(fetchCoordinatePendingComplexes({
      limit: 25,
      offset: 50,
      adminAccessCode: 'test-admin',
    })).resolves.toEqual([
      {
        parcelId: 1001,
        complexId: 501,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      resolveApiUrl('/api/v1/admin/coordinates/pending?limit=25&offset=50'),
  ...
  it('approved override를 documented admin endpoint에 PUT한다', async () => {
    ...
    await expect(approveCoordinateOverride(
      '1168010300101400001',
      {
        latitude: 37.5123,
        longitude: 127.0456,
