...
| `recommendation` | “영등포구 500세대 이상, 학원 우선 후보” | 제한 지원 | 부동산 A, Sbiz 학원 B, 철도 A, 학교 A, 좌표 확인 대규모점포 A | A | v1 결정형 정책 또는 v2 bounded 후보 안의 AI 선택, 검증된 강점·tradeoff | 후보군 밖 선택, 절대평가, 수익 보장, LLM 임의 수치·점수 | 사용한 모든 필수 dataset 통과 | 비활성 조건과 후보 0건 이유 설명 |
...
Agentic v2는 4~6의 구조 검증을 유지하면서 최종 후보·순서만 AI가 결정한다. 후보군 밖
ID, hard constraint 위반, 중복 ID, 존재하지 않는 `factId`는 응답을 거부한다.
...
2026-07-21 `comparison`은 가격·거래 표본·세대수·사용승인일·철도와 사용자가 명시한
학교/학원 항목에 한해 `제한 지원`으로 활성화했다. 누적 property allowlist는
`complex_identity,recent_trade_lookup,price_trend,recommendation,comparison`이며 직전
recommendation 값은 rollback으로 유지한다. 송파구 상위 행정구역을 descendant
