...
운영 `home_search.ai_read` 전체 실행과 실제 OpenAI live 1건은 실행하지 않았으므로
부동산 Capability를 아직 `지원`으로 활성화하지 않는다.
...
| 운영 `home_search.ai_read` offline catalog | not run |
...
- 운영 DB의 잠실엘스 식별 결과, 2026년 거래 범위, 최신 거래일이 catalog 전제와
  일치하는지 아직 확인하지 않았다.
...
1. 운영 reader DSN으로 offline catalog 전체를 실행해 `Pass` 결과를 보존한다.
2. 사용자가 비용 발생을 별도로 승인한 뒤 대표 live case 1건을 실행한다.
