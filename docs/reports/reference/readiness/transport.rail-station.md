...
- 중단: provider 필수 ID 중복·row 기준일 혼재, S3 byte 복구·공간 query·chatbot golden 미검증
...
`1900-01-00` 6행을 포함했다. `DUPLICATE_UNIQUE_KEY`, `SOURCE_DATE_MIXED`,
`REJECTED_ROW_RATIO_EXCEEDED`로 publication을 차단했고 active pointer는 없다. provider
정정 또는 dataset-specific identity/date 계약 없이는 임의 dedupe·최신행 선택을 하지
않는다.
