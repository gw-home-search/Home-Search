...
versions_through_41="$(jq '. + [41]' <<<"${versions_through_40}")"
...
jq -e --arg phase "${phase}" --argjson v39 "${versions_through_39}" --argjson v40 "${versions_through_40}" --argjson v41 "${versions_through_41}" '
  .status == "pass"
  and (if $phase == "before" then ([.history[].version] == $v39 or [.history[].version] == $v40 or [.history[].version] == $v41)
       else [.history[].version] == $v41 end)
...
if [[ "${phase}" == after ]]; then
  ...
  jq -e --slurp '.[0].data == .[1].data' "${tmp_dir}/before.json" "${snapshot}" >/dev/null || {
    echo '상태: Fail - V41 전후 row count 또는 식별자 checksum이 변경됐습니다.' >&2
    exit 1
  }
  previous_version="$(jq -er '.previous_version | select(. == 39 or . == 40 or . == 41)' "${tmp_dir}/before.json")"
else
  previous_version="$(jq -er '.history[-1].version | select(. == 39 or . == 40 or . == 41)' "${snapshot}")"
fi
enriched="${tmp_dir}/${phase}-enriched.json"
jq --arg phase "${phase}" --argjson previous_version "${previous_version}" '
  . + {phase:$phase,previous_version:$previous_version,target_version:41,failed:0,missing:0,out_of_order:0}
' "${snapshot}" >"${enriched}"
mv "${enriched}" "${snapshot}"
aws s3 cp "${snapshot}" "${prefix}/${phase}.json" --sse aws:kms --sse-kms-key-id alias/aws/s3 --only-show-errors
echo "상태: Pass - property V${previous_version}→V41 ${phase} row count, 식별자 checksum, Flyway history를 확인했습니다."
