...
cat >"${MOCK_CAPTURE_DIR}/query-${count}.sql"
if [[ "${count}" == 1 ]]; then
  cat "${MOCK_RTMS_JSON}"
else
  cat "${MOCK_NEWS_JSON}"
...
grep -Fq "execution_correlation_id IN (:'first_id'::uuid, :'repeat_id'::uuid)" "${work}/captured/query-1.sql"
grep -Fq "period_type IN ('DAILY', 'ROLLING_7D')" "${work}/captured/query-1.sql"
