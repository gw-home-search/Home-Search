...
jq -n --argjson images "${images}" --argjson platform "${platform}" '{format_version:2,tag:"v1.0.11",commit_sha:("a"*40),images:$images,platform_images:$platform,sbom_set_sha256:("b"*64),vulnerability_set_sha256:("c"*64),vulnerability_critical_gate_passed:true,vulnerability_policy_gate_passed:true}' >"${evidence}/release-manifest.json"
