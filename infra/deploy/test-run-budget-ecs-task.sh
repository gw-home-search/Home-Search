...
bash -n "${script}"
for expected in '--launch-type EC2' '--count 1' '--started-by' '--overrides' 'containerOverrides' 'property-flyway override는 exact V40 validate만 허용합니다.' 'scheduled-backup override는 release별 property search audit만 허용합니다.' 'RTMS 수동 실행은 canonical requestId 하나만 허용합니다.' '뉴스 bootstrap은 BOOTSTRAP canonical requestId만 허용합니다.' 'policyVersion=NEWS_V5' 'deadline=$((SECONDS + 7200))' 'sleep 15' 'all(.tasks[0].containers[]; .exitCode == 0)' 'aws ecs stop-task' 'completed=false'; do
