...
ai_supervisor_graph_canary_percent="$(jq -er '
  ...
  | select(type == "number" and . >= 0 and . <= 100 and floor == .)
  | tostring
' "${live_application_settings_json}")"

violations="$(jq -c \
  --arg ai_supervisor_graph_mode "${ai_supervisor_graph_mode}" \
  --arg ai_supervisor_graph_canary_percent "${ai_supervisor_graph_canary_percent}" '
  def immutable_image:
    type == "string"
  ...
  def normalized_containers($address):
    ...
    | map(
        ...
        | if $address == "aws_ecs_task_definition.application[\"ai\"]" and .name == "ai" then
            .environment |= map(select(
              .name != "HOME_AI_SUPERVISOR_GRAPH_MODE"
              and .name != "HOME_AI_SUPERVISOR_GRAPH_CANARY_PERCENT"
            ))
          else . end
        | .environment |= map(
  ...
  def ai_after_preserves_live:
    ((.change.after.container_definitions | fromjson?) // []) as $containers
    | ([ $containers[] | select(.name == "ai") ] | length) == 1
    and (([ $containers[] | select(.name == "ai") | .environment[]?
      | select(.name == "HOME_AI_SUPERVISOR_GRAPH_MODE") | .value ]
      == [$ai_supervisor_graph_mode])
    and ([ $containers[] | select(.name == "ai") | .environment[]?
      | select(.name == "HOME_AI_SUPERVISOR_GRAPH_CANARY_PERCENT") | .value ]
      == [$ai_supervisor_graph_canary_percent]));
  def safe_task_revision:
    ...
    and ($change.address != "aws_ecs_task_definition.application[\"ai\"]"
      or ($change | ai_after_preserves_live))
