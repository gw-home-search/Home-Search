  ...
  content = jsonencode({
    ...
    mainSteps = [{
      ...
      inputs = { timeoutSeconds = "900", runCommand = [
        ...
        "DOCKER_CONFIG=$(mktemp -d \"$workdir/.docker-config.XXXXXX\")",
        "export DOCKER_CONFIG",
        "aws ecr get-login-password --region '${var.aws_region}' | docker login --username AWS --password-stdin '${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com' >/dev/null",
