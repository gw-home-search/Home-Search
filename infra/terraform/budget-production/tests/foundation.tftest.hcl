...
run "data_phase_keeps_platform_services_dark_before_secret_bootstrap" {
  ...
  assert {
    condition = (
      ...
      && alltrue([
        for name in ["market-news-major-selection", "market-news-general", "market-news-major-complex", "market-news-quality-sample", "market-news-retention"] :
        one([
      ...
      && alltrue([
        ...
        one([
          for item in local.one_shot_specs[name].environment :
          item.value if item.name == "HOME_NEWS_NAVER_PROVIDER_MODE"
        ]) == "DEVELOPERS"
        && one([
          for item in local.one_shot_specs[name].environment :
          item.value if item.name == "HOME_NEWS_NAVER_BASE_URL"
        ]) == "https://openapi.naver.com"
        && one([
          for item in local.one_shot_specs[name].environment :
          item.value if item.name == "HOME_NEWS_NAVER_PATH"
        ]) == "/v1/search/news.json"
      ])
      && one([
        for item in local.one_shot_specs["rtms-daily-refresh"].environment :
