...
public final class Rs256JwtCodec {
    ...
    public String issue(JwtIssueRequest request, PrivateKey privateKey) {
        if (request == null || privateKey == null) throw new IllegalArgumentException("request and privateKey are required");
        Instant issuedAt = clock.instant();
        return Jwts.builder()
            .header().keyId(request.keyId()).and()
        ...
        try {
            Jws<Claims> parsed = Jwts.parser()
                .keyLocator(header -> resolveKey(
                    header.get("kid") instanceof String keyId ? keyId : null,
                    header.getAlgorithm(), policy))
            ...
            return new VerifiedJwt(parsed.getHeader().getKeyId(), claims.getIssuer(), audience,
                required(claims.getSubject()), required(claims.getId()), issuedAt, expiresAt, customClaims);
    ...
    private PublicKey resolveKey(String keyId, String algorithm, JwtVerificationPolicy policy) {
        if (!"RS256".equals(algorithm) || keyId == null || keyId.isBlank()) throw new UnsupportedJwtException("unsupported token header");
